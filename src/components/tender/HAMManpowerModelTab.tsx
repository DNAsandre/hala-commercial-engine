          ...
          {staffing.map((row, i) => (
            <div key={i} className="border border-border rounded-lg p-3 bg-card space-y-2 relative">
              ...
              <div className="grid grid-cols-4 gap-2">
                ...
                <div><label className="text-[9px] font-semibold text-muted-foreground mb-0.5 block">Required From</label><input type="date" className="w-full border border-border rounded px-2 py-1.5 text-xs bg-card" value={row.required_from} onChange={e => upStaff(i, "required_from", e.target.value)} onBlur={e => { const value = e.currentTarget.value; if (row.required_from !== value) upStaff(i, "required_from", value); }} /></div>
        ...
        <CardContent className="p-4 space-y-3">
          ...
          {mobilization.map((row, i) => (
            <div key={i} className="border border-border rounded-lg p-3 bg-card space-y-2 relative">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div><label className="text-[9px] font-semibold text-muted-foreground mb-0.5 block">Due Date</label><input type="date" className="w-full border border-border rounded px-2 py-1.5 text-xs bg-card" value={row.due_date} onChange={e => upMob(i, "due_date", e.target.value)} onBlur={e => { const value = e.currentTarget.value; if (row.due_date !== value) upMob(i, "due_date", value); }} /></div>
