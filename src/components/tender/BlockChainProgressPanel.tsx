...
export default function BlockChainProgressPanel({ progress, result, onCancel, onDone }: Props) {
  ...
  return (
    <Card className="border-[#075eea]/20 bg-[#075eea]/10 shadow-none">
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#075eea]/15">
            <Sparkles className="h-4 w-4 text-[#075eea] animate-pulse" />
          </div>
          <div className="flex-1">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="outline" className="text-[10px] border-[#075eea]/30 text-[#075eea]">
                {progress.current} / {progress.total}
              </Badge>
            </div>

            {/* Progress bar */}
            <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-[#075eea]/15">
              <div
                className="h-full rounded-full bg-[#075eea]/100 transition-all duration-500 ease-out"
                style={{ width: `${pct}%` }}
              />
            </div>
            ...
            <div className="mt-2 flex items-center gap-2">
              {isGenerating ? (
                <Loader2 className="h-3 w-3 animate-spin text-[#075eea]" />
              ) : progress.status === "completed" ? (
                <CheckCircle2 className="h-3 w-3 text-emerald-600" />
              ) : (
                <XCircle className="h-3 w-3 text-red-500" />
              )}
              <span className="text-[11px] text-[#075eea]">
