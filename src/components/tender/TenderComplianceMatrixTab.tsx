 ...
 * Shows compliance register, filters, summary cards, risk indicators, and review modal.
 */
      ...
      <div className="relative bg-background rounded-xl border shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div className="p-5 border-b">
          <div className="flex items-start justify-between">
            <div>
              <h3 className="text-sm font-serif font-bold">Compliance Review</h3>
...
export default function TenderComplianceMatrixTab({ ws, tenderId, reload }: { ws: TenderWorkspace; tenderId: string; reload: () => void }) {
  ...
  const statusOptions: { value: string; label: string }[] = [
    ...
    { value: "accepted_risk_mock", label: "Accepted Risk" },
  ];
  ...
  return (
    <div className="space-y-4">
              ...
              return (
                <tr key={item.id} className={`border-t border-border hover:bg-muted/30 ${isRisky ? "bg-red-50/40 border-l-2 border-l-red-400" : isWarning ? "bg-amber-50/30 border-l-2 border-l-amber-300" : ""}`}>
                  ...
                  <td className="px-3 py-2 text-center">
                    {item.legalReviewRequired ? <Badge variant="outline" className="text-[9px] text-[#075eea] bg-[#075eea]/10 border-[#075eea]/20">Required</Badge> : <span className="text-muted-foreground">—</span>}
                  </td>
                  <td className="px-3 py-2 text-center">
                  ...
                  <td className="px-3 py-2 text-center">
                    <Button variant="ghost" size="sm" className="h-7 text-[10px] gap-1" onClick={() => setReviewItem(item)}><Eye className="w-3 h-3" /> Review</Button>
                  </td>
                </tr>
              );
            })}
      ...
      {/* Tender-local advisory note */}
      <div className="p-3 rounded-lg border border-blue-200 bg-blue-50/50 dark:bg-blue-950/20 flex items-center gap-2">
        <Info className="w-3.5 h-3.5 text-blue-600 shrink-0" />
        <p className="text-[10px] text-blue-700">Partial, non-compliant, and clarification-required items are tracked as tender-local advisory indicators for review.</p>
      </div>
