      ...
      <div className="relative bg-background rounded-xl border shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div className="p-5 border-b">
          <div className="flex items-start justify-between">
            <div>
              <h3 className="text-sm font-serif font-bold">Audit Event Detail</h3>
            </div>
        ...
        <div className="p-5 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Event Code</label>
              <p className="text-xs font-mono mt-1">{entry.eventCode || entry.action}</p>
            </div>
            <div>
              <label className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Event Name</label>
              <p className="text-xs mt-1">{entry.eventName || entry.action}</p>
            </div>
          </div>
          <div>
            <label className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Description</label>
            <p className="text-xs mt-1 text-muted-foreground">{entry.details}</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Entity</label>
              <p className="text-xs mt-1">{entry.entityName || entry.entityType}</p>
            </div>
            <div>
              <label className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Category</label>
              <p className="text-xs mt-1">{getAuditCategoryLabel(entry.category)}</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Before State</label>
              <p className="text-xs mt-1 font-mono">{entry.beforeState || "-"}</p>
            </div>
            <div>
              <label className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">After State</label>
              <p className="text-xs mt-1 font-mono">{entry.afterState || "-"}</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Actor</label>
              <p className="text-xs mt-1">{entry.userName}{entry.role ? ` - ${entry.role}` : ""}</p>
            </div>
            <div>
              <label className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Timestamp</label>
              <p className="text-xs mt-1 font-mono">{new Date(entry.timestamp).toLocaleString()}</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="mt-1">
                {entry.severity ? (
                  <Badge variant="outline" className={`text-[9px] ${getSeverityColor(entry.severity)}`}>
                    {entry.severity}
                  </Badge>
                ) : (
            ...
            <div>
              <label className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Trace ID</label>
              <p className="text-xs font-mono mt-1">{entry.traceId || "-"}</p>
            </div>
          </div>
          {entry.notes && (
            <div>
              <label className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Notes</label>
              <p className="text-xs mt-1 text-muted-foreground">{entry.notes}</p>
            </div>
          )}
        </div>
...
export default function TenderAuditTrailTab({ ws }: { ws: TenderWorkspace }) {
  ...
  const subEvents = entries.filter(e => {
    const c = e.category?.toLowerCase();
    return c === "submission" || c === "split_output";
  }).length;
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
        ...
        <SummaryCard label="Bypass Events" value={bypassEvents} color="text-orange-600" />
            ...
            {filtered.map(e => {
              ...
              return (
                <tr key={e.id} className={`border-t border-border hover:bg-muted/30 ${isHighCrit ? "bg-red-50/30 border-l-2 border-l-red-400" : isBypass ? "bg-orange-50/20 border-l-2 border-l-orange-300" : ""}`}>
                  ...
                  <td className="px-3 py-2 text-center">
                    <Button variant="ghost" size="sm" className="h-7 text-[10px] gap-1" onClick={() => setSelectedEntry(e)}>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={10} className="text-center py-8 text-muted-foreground">
                  {entries.length === 0 ? "No audit events recorded yet." : "No audit events match current filters."}
                </td>
              </tr>
            )}
          </tbody>
