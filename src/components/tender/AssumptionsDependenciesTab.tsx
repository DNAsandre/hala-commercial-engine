 ...
 * key (assumptions_dependencies). The write layer patch-merges, so sibling tabs' keys are
 * preserved by the STORED facet — never re-sent from a page-load copy.
 * Exported pure for direct testing.
 */
export function buildAssumptionsDependenciesPatch(
  ...
  return { assumptions_dependencies: { assumptions, dependencies, exclusions, clarifications, recommendation } };
}
...
export default function AssumptionsDependenciesTab({ ws, onOpenDocuments, onOpenGlobalIntel, onSaved }: Props) {
    ...
    try {
      await runTenderTabSave({
        write: expectedRevision =>
          updateTenderSolutionDesignData(tenderId, buildAssumptionsDependenciesPatch(assumptions, dependencies, exclusions, clarifications, recommendation), {
            expectedRevision,
            reason: "Assumptions & Dependencies saved",
          }),
        revisionToken: tenderRevisionTokenOf(ws),
        staleRetryArmed,
        labels: { saved: "Assumptions & Dependencies saved", failed: "Save failed" },
        onConfirmed: () => onSaved?.(),
  ...
  }, [tenderId, assumptions, dependencies, exclusions, clarifications, recommendation, onSaved, ws]);
        ...
        <CardContent className="p-4 space-y-3">
          ...
          {dependencies.map((row, i) => (
            <div key={i} className="border border-border rounded-lg p-3 bg-card space-y-2 relative">
              ...
              <div className="grid grid-cols-4 gap-2">
                <div><label className="text-[9px] font-semibold text-muted-foreground mb-0.5 block">Due Date</label><input type="date" className="w-full border border-border rounded px-2 py-1.5 text-xs bg-card" value={row.due_date} onChange={e => upD(i, "due_date", e.target.value)} onBlur={e => { const value = e.currentTarget.value; if (row.due_date !== value) upD(i, "due_date", value); }} /></div>
