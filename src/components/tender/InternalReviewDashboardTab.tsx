...
export default function InternalReviewDashboardTab({ ws, onOpenDocuments, onOpenGlobalIntel }: Props) {
  ...
  const [activeSection, setActiveSection] = useState<DashboardSection>("departments");
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className={`gap-0 overflow-hidden rounded-lg border-border py-0 shadow-none ${activeSection !== "departments" ? "hidden" : ""}`}>
        <CardHeader className="p-0"><SectionHeader title="Department Summary" icon={<Shield className="w-3.5 h-3.5 text-[#075eea]" />} /></CardHeader>
      ...
      {/* ── 2. Overall Readiness ── */}
      <Card className={`gap-0 overflow-hidden rounded-lg border-border py-0 shadow-none ${activeSection !== "readiness" ? "hidden" : ""}`}>
        <CardHeader className="p-0"><SectionHeader title="Overall Review Readiness" icon={<BarChart3 className="w-3.5 h-3.5 text-[#075eea]" />} /></CardHeader>
        <CardContent className="p-4">
