 ...
 *
 * AI suggestions are limited to current tender context. No mock data. No document handoff mutation.
 */
...
import { TenderStageTaskShell } from "./TenderStageTaskShell";
...
interface TOCSection {
  ...
  document_assembly_target: string;
  owner: string;
...
const LEGACY_DOCUMENT_TARGET_KEY = ["pdf", "studio", "target"].join("_");

type TocSectionKey = "source" | "summary" | "builder" | "blocks";

const TOC_SECTION_TABS: { key: TocSectionKey; label: string; icon: ReactNode }[] = [
  { key: "source", label: "Source Intelligence", icon: <BarChart3 className="w-3.5 h-3.5" /> },
  { key: "summary", label: "TOC Summary", icon: <ClipboardList className="w-3.5 h-3.5" /> },
  { key: "builder", label: "TOC Builder", icon: <Layers className="w-3.5 h-3.5" /> },
  { key: "blocks", label: "Create Blocks", icon: <Layers className="w-3.5 h-3.5" /> },
];
...
function readDocumentAssemblyTarget(raw: Record<string, unknown> | null | undefined): string {
  if (!raw) return "";
  return String(raw.document_assembly_target ?? raw[LEGACY_DOCUMENT_TARGET_KEY] ?? "");
}

function normalizeTocSection(raw: any, index: number): TOCSection {
  const section: TOCSection = {
    ...emptySection(String(index + 1)),
    ...(raw || {}),
    id: String(raw?.id || nanoid(8)),
    section_number: String(raw?.section_number || index + 1),
    section_title: String(raw?.section_title || `Section ${index + 1}`),
    volume: VOLUMES.includes(raw?.volume) ? raw.volume : "Technical",
    document_assembly_target: readDocumentAssemblyTarget(raw),
  };
  delete (section as unknown as Record<string, unknown>)[LEGACY_DOCUMENT_TARGET_KEY];
  return section;
}

function normalizeTocVersion(raw: any, index: number): TOCVersion {
  return {
    id: String(raw?.id || nanoid(8)),
    version: Number(raw?.version || index + 1),
    status: String(raw?.status || "Draft"),
    sections: Array.isArray(raw?.sections) ? raw.sections.map(normalizeTocSection) : [],
...
function SourceIntelligenceSummary({ ws }: { ws: TenderWorkspace }) {
  ...
  const documentSummary = useMemo(() => summarizeTenderDocuments(docs), [docs]);
    ...
    <Card className="border-border shadow-none">
      <CardHeader className="py-2 px-4 cursor-pointer bg-muted/20 border-b border-border" onClick={() => setOpen(!open)}>
        <div className="flex items-center gap-2">
          ...
          <BarChart3 className="w-3.5 h-3.5 text-[#075eea]" />
        ...
        <CardContent className="p-3">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            ...
            <div className="space-y-0.5 rounded-md border border-border p-2">
              ...
              {row("Tender Drafting", String(documentSummary.total))}
              {row("Source", String(documentSummary.source))}
              {row("Supporting", String(documentSummary.supporting))}
            </div>
          </div>
        </CardContent>
      )}
...
// ─── Main Component ─────────────────────────────────────────
interface Props { ws: TenderWorkspace; reload: () => void; onOpenDocuments?: () => void; onOpenGlobalIntel?: () => void; onSaved?: () => void }

export default function ProposalArchitectureTOCTab({ ws, reload, onOpenDocuments, onOpenGlobalIntel, onSaved }: Props) {
  ...
  const [tocVersions, setTocVersions] = useState<TOCVersion[]>(() => {
    if (Array.isArray(saved.toc_versions) && saved.toc_versions.length > 0) {
      return saved.toc_versions.map(normalizeTocVersion);
    }
    return [{
      id: nanoid(8),
      version: 1,
  ...
  const [activeSection, setActiveSection] = useState<TocSectionKey>("summary");
  ...
  const generateTOCSuggestion = useCallback(async () => {
    ...
    try {
      ...
      const aiSections: TOCSection[] = parsed.map((s: any, i: number) => ({
        ...
        document_assembly_target: readDocumentAssemblyTarget(s),
        owner: s.owner || "",
        include_in_proposal: true,
        status: "Draft",
      }));
  ...
  const handleAutoDraftAll = useCallback(async () => {
    ...
    const newBlocks = includedSections.map(s => ({
      ...
      required_source_data: s.required_source_data,
      required_evidence: s.required_evidence,
      document_assembly_target: s.document_assembly_target,
      owner: s.owner,
  ...
  const metrics = useMemo(() => [
    { label: "TOC Status", value: tocStatus },
    { label: "Sections", value: String(sections.length) },
    { label: "Included", value: String(includedCount) },
    { label: "Blocks Created", value: blocksCreated ? "Yes" : "No" },
    { label: "Version", value: `v${activeVersion.version}` },
  ], [activeVersion.version, blocksCreated, includedCount, sections.length, tocStatus]);
  ...
  return (
    <TenderStageTaskShell
      ...
      sectionTabs={TOC_SECTION_TABS}
      stageIntelOpen={stageIntelOpen}
      onStageIntelOpenChange={setStageIntelOpen}
      metrics={metrics}
      onOpenDocuments={onOpenDocuments}
      onOpenGlobalIntel={onOpenGlobalIntel}
      saved={Boolean(drafting?.proposal_architecture && (sections.length > 0 || tocStatus !== "Draft") && !dirty)}
      unsaved={dirty}
    >
      {/* Source Intelligence Summary */}
      <div className={activeSection !== "source" ? "hidden" : ""}>
        <SourceIntelligenceSummary ws={ws} />
      </div>

      {/* TOC Workspace Header */}
      <Card className={`border-border shadow-none ${activeSection !== "summary" ? "hidden" : ""}`}>
        <CardHeader className="py-2 px-4 bg-muted/20 border-b border-border">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <ClipboardList className="w-3.5 h-3.5 text-[#075eea]" />
      ...
      <div className={`space-y-2 px-1 ${activeSection !== "builder" ? "hidden" : ""}`}>
        ...
        {aiPreview && aiPreview.length > 0 && (
          <Card className="border-[#075eea]/20 bg-[#075eea]/10 shadow-none">
            <CardHeader className="py-2 px-4 bg-[#075eea]/10 border-b border-[#075eea]/20">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Sparkles className="w-3.5 h-3.5 text-[#075eea]" />
                  <span className="text-xs font-semibold text-[#064fc4]">AI TOC Suggestion — {aiPreview.length} sections</span>
                  <Badge variant="outline" className="text-[8px] border-[#075eea]/30 text-[#075eea]">Preview</Badge>
                </div>
                <div className="flex items-center gap-2">
                  <Button size="sm" className="h-7 text-[10px] gap-1 bg-[#075eea] hover:bg-[#0655d6]" onClick={acceptAiPreview}>
                    Accept & Apply
                  </Button>
            ...
            <CardContent className="p-0">
              <table className="w-full text-[11px]">
                <thead className="bg-[#075eea]/10 border-b border-[#075eea]/20">
                  <tr>
                ...
                <tbody>
                  {aiPreview.map(s => (
                    <tr key={s.id} className="border-t border-[#075eea]/15 hover:bg-[#075eea]/10">
          ...
          <Card className="border-[#075eea]/20 bg-[#075eea]/10 shadow-none">
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#075eea]/15">
                  <Sparkles className="h-5 w-5 text-[#075eea]" />
                </div>
                <div className="flex-1">
                  <p className="text-sm font-semibold text-[#064fc4]">
                    TOC Created — {sections.filter(s => s.include_in_proposal).length} sections
                  </p>
                  <p className="mt-1 text-xs text-[#075eea]">
                    Would you like AI to auto-draft content for all {sections.filter(s => s.include_in_proposal).length} blocks?
                  </p>
                  <p className="mt-1 text-[10px] text-[#075eea] italic">
                    This will generate content for each block sequentially using the Section Writer bot. You can cancel at any time.
                  </p>
                  <div className="mt-3 flex items-center gap-2">
                    <Button size="sm" className="h-8 text-xs gap-1.5 bg-[#075eea] hover:bg-[#0655d6]" onClick={handleAutoDraftAll}>
                      <Zap className="w-3.5 h-3.5" /> {chainConfig.chain_label || "Auto-draft all blocks"}
                    </Button>
                    <Button size="sm" variant="outline" className="h-8 text-xs gap-1.5 border-[#075eea]/30 text-[#075eea] hover:bg-[#075eea]/10" onClick={handleDraftManually}>
      ...
      {/* Editable TOC Table */}
      <Card className={`border-border shadow-none ${activeSection !== "builder" ? "hidden" : ""}`}>
        <CardHeader className="py-2 px-4 bg-muted/20 border-b border-border">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-[11px]">
                ...
                <tbody>
                  {sections.map((s, i) => (
                    <Fragment key={s.id}>
                      <tr className={`border-t border-border hover:bg-muted/30 cursor-pointer ${expandedRow === s.id ? "bg-[#075eea]/10 dark:bg-[#075eea]/10" : ""}`} onClick={() => setExpandedRow(expandedRow === s.id ? null : s.id)}>
                        <td className="px-2 py-1.5 text-muted-foreground font-mono">{s.section_number}</td>
                        ...
                        <tr key={`${s.id}-detail`} className="border-t border-dashed border-border bg-muted/10">
                          <td colSpan={7} className="px-4 py-3">
                            <div className="grid grid-cols-2 gap-3">
                              ...
                              <div>
                                <label className="text-[10px] font-semibold text-muted-foreground">Document Assembly Target</label>
                                <Input className="h-8 text-xs mt-1" value={s.document_assembly_target} onChange={e => updateSection(s.id, { document_assembly_target: e.target.value })} placeholder="e.g. Section 3 - Proposed Solution" />
                              </div>
                            </div>
                          </td>
                        </tr>
      ...
      {/* Create Blocks from TOC */}
      <Card className={`border-border shadow-none ${activeSection !== "blocks" ? "hidden" : ""}`}>
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div>
