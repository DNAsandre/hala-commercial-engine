...
function LossReasonView({ ws, reload, intelMetrics, onOpenDocuments, onOpenGlobalIntel, saved }: { ws: TenderWorkspace; reload: () => void; intelMetrics: { label: string; value: string }[]; onOpenDocuments?: () => void; onOpenGlobalIntel?: () => void; saved?: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={activeSection !== "loss_details" ? "hidden" : ""}>
        <LossReasonTab ws={ws} reload={reload} />
      </div>
...
function LessonsLearnedView({ ws, reload, intelMetrics, onOpenDocuments, onOpenGlobalIntel, saved }: { ws: TenderWorkspace; reload: () => void; intelMetrics: { label: string; value: string }[]; onOpenDocuments?: () => void; onOpenGlobalIntel?: () => void; saved?: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={activeSection !== "lessons_register" ? "hidden" : ""}>
        <LessonsLearnedTab ws={ws} reload={reload} />
      </div>
...
function CompetitorIntelView({ ws, reload, intelMetrics, onOpenDocuments, onOpenGlobalIntel, saved }: { ws: TenderWorkspace; reload: () => void; intelMetrics: { label: string; value: string }[]; onOpenDocuments?: () => void; onOpenGlobalIntel?: () => void; saved?: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={activeSection !== "competitor_register" ? "hidden" : ""}>
        <CompetitorIntelTab ws={ws} reload={reload} />
      </div>
...
function RebidPotentialView({ ws, reload, intelMetrics, onOpenDocuments, onOpenGlobalIntel, saved }: { ws: TenderWorkspace; reload: () => void; intelMetrics: { label: string; value: string }[]; onOpenDocuments?: () => void; onOpenGlobalIntel?: () => void; saved?: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={activeSection !== "rebid_assessment" ? "hidden" : ""}>
        <RebidPotentialTab ws={ws} reload={reload} />
      </div>
    </div>
  );
}
...
export default function LostWithdrawnStage({ ws, activeTab, reload, onOpenDocuments, onOpenGlobalIntel }: StageProps) {
  ...
  if (activeTab === "loss_reason")
    return <LossReasonView ws={ws} reload={reload} intelMetrics={intelMetrics} onOpenDocuments={onOpenDocuments} onOpenGlobalIntel={onOpenGlobalIntel} saved={hasLoss} />;
  if (activeTab === "lessons_learned")
    return <LessonsLearnedView ws={ws} reload={reload} intelMetrics={intelMetrics} onOpenDocuments={onOpenDocuments} onOpenGlobalIntel={onOpenGlobalIntel} saved={hasLessons} />;
  if (activeTab === "competitor_intelligence")
    return <CompetitorIntelView ws={ws} reload={reload} intelMetrics={intelMetrics} onOpenDocuments={onOpenDocuments} onOpenGlobalIntel={onOpenGlobalIntel} saved={hasCompIntel} />;
  if (activeTab === "rebid_potential")
    return <RebidPotentialView ws={ws} reload={reload} intelMetrics={intelMetrics} onOpenDocuments={onOpenDocuments} onOpenGlobalIntel={onOpenGlobalIntel} saved={hasRebid} />;
