...
type TechSectionKey = "readiness" | "blocks";

const TECH_SECTION_TABS: { key: TechSectionKey; label: string; icon: ReactNode }[] = [
  { key: "readiness", label: "Technical Volume Readiness", icon: <BarChart3 className="w-3.5 h-3.5" /> },
  { key: "blocks", label: "Technical / Shared Blocks", icon: <BookOpen className="w-3.5 h-3.5" /> },
];
...
export default function TechnicalVolumeTab({ ws, onOpenDocuments, onOpenGlobalIntel }: Props) {
  ...
  const [activeSection, setActiveSection] = useState<TechSectionKey>("readiness");
  ...
  return (
    ...
    >
      ...
      <TenderStageSectionCard
        title="Technical Volume Readiness"
      ...
      <TenderStageSectionCard
        title="Technical / Shared Blocks"
        icon={<BookOpen className="h-3.5 w-3.5 text-[#075eea]" />}
        badge={`${techBlocks.length} block${techBlocks.length !== 1 ? "s" : ""}`}
        hidden={activeSection !== "blocks"}
      >
        {techBlocks.length === 0 ? (
          <p className="py-8 text-center text-sm text-muted-foreground">No Technical or Shared blocks in proposal block register.</p>
        ) : (
