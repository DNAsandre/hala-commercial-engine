/**
 * ClientBafoManagerTab — Tab 2 of Client Evaluation Stage
 *
 * Manages BAFO (Best and Final Offer) preparation:
 * - BAFO status tracking
 * - Request details & scope changes
 * - Revised pricing with GP comparison
 *
 * Data: type_details.client_evaluation.bafo
 * No AI. No mock data.
 */
...
export default function ClientBafoManagerTab({ ws, reload }: Props) {
  ...
  const [bafoStatus, setBafoStatus] = useState(saved.bafo_status || "not_requested");
  ...
  const statusColor = (s: string) => {
    ...
    if (s === "requested") return "border-amber-300 text-amber-700 bg-amber-50";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-2 text-[10px] text-muted-foreground bg-[#075eea]/10 border border-[#075eea]/15 rounded-md px-3 py-2">
        ...
        <span>Manage BAFO (Best and Final Offer) requests from the client. Track revised pricing and measure GP impact against the original submission.</span>
      </div>

      {/* Save Strip */}
      <div className="flex items-center gap-3 px-3 py-2 rounded-md border border-border bg-muted/10">
        <span className="text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">BAFO</span>
        <Badge variant="outline" className={`text-[8px] ${statusColor(bafoStatus)}`}>
          {BAFO_STATUSES.find(s => s.value === bafoStatus)?.label}
        </Badge>
      ...
      <Card className="border-border shadow-none">
        <CardHeader className="py-2 px-4 bg-muted/20 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold">BAFO Status & Request</span>
          </div>
        </CardHeader>
        <CardContent className="p-4">
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="text-[10px] font-semibold text-muted-foreground">BAFO Status</label>
              <Select value={bafoStatus} onValueChange={v => { setBafoStatus(v); mark(); }}>
                <SelectTrigger className="h-8 text-xs mt-1"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {BAFO_STATUSES.map(s => <SelectItem key={s.value} value={s.value} className="text-xs">{s.label}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            {isActive && (
              <>
                <div>
                  <label className="text-[10px] font-semibold text-muted-foreground">BAFO Received Date</label>
                  <Input type="date" className="h-8 text-xs mt-1" value={receivedDate} onChange={e => { setReceivedDate(e.target.value); mark(); }} />
                </div>
                <div>
                  <label className="text-[10px] font-semibold text-muted-foreground">BAFO Due Date</label>
          ...
          {isActive && (
            <div className="mt-4 pt-4 border-t border-border">
              ...
              <Textarea className="text-xs mt-1 min-h-[60px]" value={scopeChanges} onChange={e => { setScopeChanges(e.target.value); mark(); }} placeholder="What scope or pricing changes did the client request in the BAFO?" />
            </div>
          )}
        </CardContent>
      </Card>
      ...
      {isActive && (
        <Card className="border-border shadow-none">
          <CardHeader className="py-2 px-4 bg-muted/20 border-b border-border">
            <div className="flex items-center gap-2">
              <DollarSign className="w-3.5 h-3.5 text-[#075eea]" />
              <span className="text-xs font-semibold">BAFO Response</span>
            </div>
          </CardHeader>
          <CardContent className="p-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] font-semibold text-muted-foreground">BAFO Submitted Date</label>
            ...
            <div className="mt-4">
              ...
              <Textarea className="text-xs mt-1 min-h-[50px]" value={responseNotes} onChange={e => { setResponseNotes(e.target.value); mark(); }} placeholder="Summary of what was submitted in the BAFO response..." />
            </div>
          </CardContent>
        </Card>
      )}
