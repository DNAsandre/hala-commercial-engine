...
import {
  ...
  MessageSquare, FolderOpen, PanelRightOpen, Maximize2, Minimize2, BarChart3,
} from "lucide-react";
...
type CustomerFitSectionKey = "snapshot" | "dimensions" | "scorecard" | "evidence" | "gaps" | "recommendation";

const CUSTOMER_FIT_SECTION_TABS: { key: CustomerFitSectionKey; label: string; icon: ReactNode }[] = [
  { key: "snapshot", label: "Customer Snapshot", icon: <Building2 className="w-3.5 h-3.5" /> },
  { key: "dimensions", label: "Fit Dimensions", icon: <Target className="w-3.5 h-3.5" /> },
  { key: "scorecard", label: "Qualification Scorecard", icon: <ClipboardCheck className="w-3.5 h-3.5" /> },
  { key: "evidence", label: "Evidence Register", icon: <FileText className="w-3.5 h-3.5" /> },
  { key: "gaps", label: "Fit Gaps / Clarifications", icon: <AlertTriangle className="w-3.5 h-3.5" /> },
  { key: "recommendation", label: "Qualification Recommendation", icon: <MessageSquare className="w-3.5 h-3.5" /> },
];
...
export default function CustomerFitQualification({ ws, onOpenDocuments, onOpenGlobalIntel }: Props) {
  ...
  const [activeSection, setActiveSection] = useState<CustomerFitSectionKey>("snapshot");
  const [stageIntelOpen, setStageIntelOpen] = useState(false);
  const [tableFocus, setTableFocus] = useState(false);
  const toggle = (key: string) => setOpenSections(prev => ({ ...prev, [key]: !prev[key] }));
  const selectSection = (key: CustomerFitSectionKey) => {
    setActiveSection(key);
    setOpenSections(prev => ({ ...prev, [key]: true }));
  };

  const hasTableFocus = activeSection === "scorecard";
    ...
    <div className="space-y-4">
      <CardHeader className="hidden">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-semibold flex items-center gap-2">
            <ClipboardCheck className="w-4 h-4 text-[#075eea]" />
      ...
      <CardContent className="p-0">
        <div className="-mx-px -mt-px flex flex-wrap items-center justify-between gap-3 rounded-t-lg border-b border-[#1f3f6f] bg-[#0b1726] px-4 py-1.5">
          ...
          {!dirty && data.dimensions.some(d => d.assessment !== "Not Assessed") && <Badge variant="outline" className="h-7 rounded-md border-emerald-400 bg-emerald-500/15 px-2.5 text-[11px] font-medium text-emerald-200">Saved</Badge>}
          </div>
        </div>
        ...
        {stageIntelOpen && (
          <div className="border-b border-border bg-card p-4">
            <div className="grid gap-3 sm:grid-cols-4">
              <StageIntelMetric label="Score" value={`${scorecard.total}/${scorecard.maxScore}`} />
              <StageIntelMetric label="Status" value={scorecard.status} />
              <StageIntelMetric label="Assessed Dimensions" value={`${data.dimensions.filter(d => d.assessment !== "Not Assessed").length}/${data.dimensions.length}`} />
              <StageIntelMetric label="Gaps / Clarifications" value={`${data.gaps.length}`} />
            </div>
            <div className="mt-3">
        ...
        <QualificationInnerTabs tabs={CUSTOMER_FIT_SECTION_TABS} activeKey={activeSection} onSelect={selectSection} />
        <div className="p-4 space-y-4">

        {/* ── 1. Customer Snapshot ──────────────────────────── */}
        <div className={`overflow-hidden rounded-lg border border-border bg-card shadow-none [&>div:not(:first-child)]:p-4 ${activeSection !== "snapshot" ? "hidden" : ""}`}>
        <SectionHeader title="Customer Snapshot" icon={<Building2 className="w-3.5 h-3.5 text-[#075eea]" />} open={openSections.snapshot} onToggle={() => toggle("snapshot")} />
        {openSections.snapshot && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {([
              ...
              <div key={f.key} className="space-y-0.5">
                ...
                {"readonlyVal" in f ? (
                  ...
                  <input
                    className="w-full text-xs border border-border rounded px-2 py-1.5 bg-card focus:outline-none focus:ring-1 focus:ring-[#075eea]/40"
            ...
            <div className="col-span-full space-y-0.5">
              ...
              <textarea
                className="w-full text-xs border border-border rounded px-2 py-1.5 bg-card focus:outline-none focus:ring-1 focus:ring-[#075eea]/40 min-h-[48px]"
                rows={2}
        ...
        {/* ── 2. Fit Dimensions ─────────────────────────────── */}
        <div className={`overflow-hidden rounded-lg border border-border bg-card shadow-none [&>div:not(:first-child)]:p-4 ${activeSection !== "dimensions" ? "hidden" : ""}`}>
        <SectionHeader title="Fit Dimensions" icon={<Target className="w-3.5 h-3.5 text-[#075eea]" />} open={openSections.dimensions} onToggle={() => toggle("dimensions")}
        ...
        <div className={`overflow-hidden rounded-lg border border-border bg-card shadow-none [&>div:not(:first-child)]:p-4 ${activeSection !== "scorecard" ? "hidden" : ""}`}>
        <SectionHeader title="Qualification Scorecard" icon={<ClipboardCheck className="w-3.5 h-3.5 text-[#075eea]" />} open={openSections.scorecard} onToggle={() => toggle("scorecard")}
          badge={scorecard.status === "Not Assessed" ? "Not Assessed" : `${scorecard.total}/${scorecard.maxScore}`}
        />
        {openSections.scorecard && (
          <div className={`border border-border rounded-lg overflow-hidden ${tableFocus ? "mx-[-12px]" : ""}`}>
        ...
        {/* ── 4. Evidence Register ──────────────────────────── */}
        <div className={`overflow-hidden rounded-lg border border-border bg-card shadow-none [&>div:not(:first-child)]:p-4 ${activeSection !== "evidence" ? "hidden" : ""}`}>
        <SectionHeader title="Evidence Register" icon={<FileText className="w-3.5 h-3.5 text-[#075eea]" />} open={openSections.evidence} onToggle={() => toggle("evidence")}
        ...
        <div className={`overflow-hidden rounded-lg border border-border bg-card shadow-none [&>div:not(:first-child)]:p-4 ${activeSection !== "gaps" ? "hidden" : ""}`}>
        ...
        {/* ── 6. Qualification Recommendation ───────────────── */}
        <div className={`overflow-hidden rounded-lg border border-border bg-card shadow-none [&>div:not(:first-child)]:p-4 ${activeSection !== "recommendation" ? "hidden" : ""}`}>
        <SectionHeader title="Qualification Recommendation" icon={<MessageSquare className="w-3.5 h-3.5 text-[#075eea]" />} open={openSections.recommendation} onToggle={() => toggle("recommendation")} />
...
function SectionHeader({ title, icon, open, onToggle, badge }: {
  ...
  return (
    <button
      type="button"
      className="flex items-center gap-2 w-full border-b border-border bg-muted/20 px-4 py-3 text-left group"
      onClick={onToggle}
    >
