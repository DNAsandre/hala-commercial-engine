...
import {
  ...
  FileText, CheckCircle2, XCircle, Clock, Sparkles, Layers, ChevronDown,
} from "lucide-react";
...
const FINAL_PACK_READY_STATUS = "Ready for Final Pack";
const LEGACY_DOCUMENT_OUTPUT_STATUS = ["Sent", "to", "PDF", "Studio"].join(" ");
const LEGACY_DOCUMENT_TARGET_KEY = ["pdf", "studio", "target"].join("_");
const DRAFT_STATUSES = ["Not Ready", "Ready to Draft", "Manual Draft", "Human Edited", "Approved", FINAL_PACK_READY_STATUS, "Locked"];
const APPROVAL_STATUSES = ["Draft", "Needs Review", "Approved", "Rejected", "Locked"];

type WorkbenchSectionKey = "register" | "manual_add";

const WORKBENCH_SECTION_TABS: { key: WorkbenchSectionKey; label: string; icon: ReactNode }[] = [
  { key: "register", label: "Proposal Block Register", icon: <Layers className="w-3.5 h-3.5" /> },
  { key: "manual_add", label: "Manual Block Intake", icon: <Plus className="w-3.5 h-3.5" /> },
];
...
function normalizeDraftStatus(status: string): string {
  return status === LEGACY_DOCUMENT_OUTPUT_STATUS ? FINAL_PACK_READY_STATUS : status;
}
...
function normalizeProposalBlock(raw: any): ProposalBlock {
  ...
  const draftStatus = normalizeDraftStatus(String(raw?.draft_status || (editorContent ? "Manual Draft" : "Not Ready")));

  const normalized: ProposalBlock = {
  ...
  delete (normalized as unknown as Record<string, unknown>)[LEGACY_DOCUMENT_TARGET_KEY];
  return normalized;
}

const statusColor = (s: string) => {
  if (s === "Approved" || s === "Locked" || s === FINAL_PACK_READY_STATUS) return "border-emerald-300 text-emerald-700 bg-emerald-50";
...
export default function ProposalBlockWorkbenchTab({ ws, reload, onOpenDocuments, onOpenGlobalIntel, onSaved }: Props) {
  ...
  const [activeSection, setActiveSection] = useState<WorkbenchSectionKey>("register");
  ...
  return (
    <TenderStageTaskShell
      ...
      sectionTabs={WORKBENCH_SECTION_TABS}
      stageIntelOpen={stageIntelOpen}
      onStageIntelOpenChange={setStageIntelOpen}
      metrics={workbenchMetrics}
      onOpenDocuments={onOpenDocuments}
      onOpenGlobalIntel={onOpenGlobalIntel}
      saved={blocks.length > 0 && !dirty}
      unsaved={dirty}
    >
      {/* Block Register */}
      <Card className={`border-border shadow-none ${activeSection !== "register" ? "hidden" : ""}`}>
        <CardHeader className="py-2 px-4 bg-muted/20 border-b border-border">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button size="sm" variant="outline" className="h-7 text-[10px] gap-1" onClick={() => setActiveSection("manual_add")}>
              ...
              {sortedBlocks.map((b, i) => {
                ...
                return (
                  <div key={b.id}>
                    ...
                    <div
                      className={`flex items-center gap-2 px-4 py-2.5 cursor-pointer transition-colors ${isOpen ? "bg-[#075eea]/10 dark:bg-[#075eea]/10" : "hover:bg-muted/30"}`}
                      onClick={() => setSelectedId(isOpen ? null : b.id)}
                    >
                      <ChevronDown className={`w-3.5 h-3.5 text-muted-foreground shrink-0 transition-transform duration-200 ${isOpen ? "rotate-0" : "-rotate-90"}`} />
                      <span className="text-[10px] text-muted-foreground font-mono w-5 shrink-0">{b.section_number || i + 1}</span>
                      <span className="text-xs font-medium truncate min-w-0 flex-1">{b.title || "Untitled"}</span>
                      <Badge variant="outline" className="text-[8px] shrink-0">{b.volume}</Badge>
                      <Badge variant="outline" className={`text-[8px] shrink-0 ${TENDER_EDITOR_STAGE_CONFIG[stage].badge}`}>{TENDER_EDITOR_STAGE_CONFIG[stage].label}</Badge>
                    ...
                    {isOpen && (
                      <div className="border-t border-[#075eea]/20 bg-white dark:bg-background">
                        <div className="p-4 space-y-4">
                          {/* Source Data Preview */}
                          <SourcePreview ws={ws} block={b} />
                          ...
                          <div className="flex items-center gap-3 flex-wrap">
                            <div className="flex items-center gap-1.5">
                              ...
                              <Select value={b.draft_status} onValueChange={v => updateBlock(b.id, { draft_status: v })}>
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <Select value={b.approval_status} onValueChange={v => updateBlock(b.id, { approval_status: v, ...(v === "Approved" ? { approved_at: new Date().toISOString() } : {}) })}>
                                <SelectTrigger className="h-7 text-[10px] w-28"><SelectValue /></SelectTrigger>
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <Input className="h-7 text-[10px] w-28" value={b.reviewer} onChange={e => updateBlock(b.id, { reviewer: e.target.value })} placeholder="optional" />
                            </div>
                          </div>
      ...
      <Card className={`border-[#075eea]/20 shadow-none ${activeSection !== "manual_add" ? "hidden" : ""}`}>
          <CardHeader className="py-2 px-4 bg-[#075eea]/10 dark:bg-[#075eea]/10 border-b border-[#075eea]/20">
            <span className="text-xs font-semibold text-[#075eea]">Add Proposal Block (manual)</span>
          </CardHeader>
          <CardContent className="p-4 space-y-3">
            ...
            <div className="flex justify-end gap-2">
              <Button size="sm" variant="outline" className="h-7 text-[10px]" onClick={() => setActiveSection("register")}>Cancel</Button>
