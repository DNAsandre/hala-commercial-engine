...
export default function NegotiationChangesTab({ ws, reload, onDirtyChange }: Props) {
  ...
  const handleSave = useCallback(async () => {
    setSaving(true);
    try {
      const res = await updateTenderNegotiationData(tenderId, "requested_changes", items, `${items.length} items`, wsRevisionToken(ws));
      // P2a threading + honest outcome; stale keeps the entry on screen.
      if (!reportSaveOutcome(res, "Requested changes saved.")) return;
      setDirty(false); onDirtyChange?.(false); reload();
    } catch (e: any) { toast.error(e.message || "Save failed."); }
    finally { setSaving(false); }
  }, [items, tenderId, reload, ws, onDirtyChange]);
