  ...
  return (
    <div className="space-y-1">
      ...
      <Section title="Commercial Indicators" defaultOpen
        icon={<Target className="w-4 h-4 text-[#0b73ff]" />}
      >
        <div className="space-y-0.5">
  ...
  return (
    <div className="space-y-1">
      ...
      <Section title="Competitive Landscape" defaultOpen={false}>
        ...
        {/* Human-entered competitive context only. Later automation belongs in the approved builder path. */}
        <div className="mt-4 pt-4 border-t border-border/50">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-3">Competitive Notes</p>
          <FieldRow label="Findings">
            <FieldTextarea
              value={data.strategicFindings}
              onChange={v => onChange({ ...data, strategicFindings: v })}
              placeholder="Human-entered competitor, incumbent, relationship, or strategic notes."
              rows={4}
            />
          </FieldRow>
        </div>
...
export function OpportunityBriefTab({
  ...
  return (
    <div className="space-y-1">
      <Section title="Customer Context" defaultOpen icon={<BookOpen className="w-4 h-4 text-[#0b73ff]" />}>
        <div className="space-y-0.5">
            ...
            <div key={item.key} className={`flex items-start gap-3 p-3 rounded-lg border transition-all ${
              ...
              <div className="flex-1 min-w-0">
                ...
                <input
                  ...
                  placeholder="Notes..."
                  className="w-full mt-1 px-2 py-1 text-xs rounded border border-border bg-background focus:outline-none focus:ring-1 focus:ring-[#075eea]/40"
                />
              </div>
            </div>
          ))}
