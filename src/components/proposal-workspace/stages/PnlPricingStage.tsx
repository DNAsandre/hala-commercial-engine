...
export function PnlCalculatorTab({
  ...
  const commitVersionsAndActive = (
    nextVersions: PnlVersion[],
    nextActiveId: string
  ) => {
    if (onVersionsAndActiveChange) {
      onVersionsAndActiveChange(nextVersions, nextActiveId);
      return;
    }
  ...
  const createVersion = () => {
    ...
    commitVersionsAndActive([...versions, v], v.id);
  };
  ...
  const addLine = (type: "revenue" | "costs") => {
    if (!active) return;
    const lines = [...active[type], { label: "", amount: 0 }];
    onVersionsChange(
      versions.map(v => (v.id === active.id ? { ...v, [type]: lines } : v))
    );
  };
  ...
  const updateField = (field: string, val: any) => {
    if (!active) return;
    onVersionsChange(
      versions.map(v => (v.id === active.id ? { ...v, [field]: val } : v))
    );
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {!active ? (
        <SignalCard
          type="info"
          message="No P&L version created"
          recommendation="Create a P&L version to establish commercial baseline"
        />
      ) : (
        <>
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              {
                label: "Total Cost",
                value: formatSAR(totalCostWithOH),
                color: "",
              },
              {
                label: "Gross Profit",
                value: formatSAR(gp),
                color: gp >= 0 ? "text-emerald-700" : "text-red-700",
              },
              {
                label: "GP%",
              ...
              {active.revenue.map((l, i) => (
                <div key={i} className="flex items-center gap-3">
                  <input
                    value={l.label}
                    onChange={e => {
                      const lines = [...active.revenue];
                      lines[i] = { ...l, label: e.target.value };
                      updateField("revenue", lines);
                    }}
                  ...
                  <input
                    ...
                    value={l.amount || ""}
                    onChange={e =>
                      updateLine("revenue", i, Number(e.target.value))
                    }
              ...
              {active.costs.map((l, i) => (
                <div key={i} className="flex items-center gap-3">
                  <input
                    value={l.label}
                    onChange={e => {
                      const lines = [...active.costs];
                      lines[i] = { ...l, label: e.target.value };
                      updateField("costs", lines);
                    }}
                    className="flex-1 px-2 py-1.5 text-sm rounded border border-border bg-background"
                  />
                  <input
                    ...
                    value={l.amount || ""}
                    onChange={e =>
                      updateLine("costs", i, Number(e.target.value))
                    }
                    className="w-32 px-2 py-1.5 text-sm rounded border border-border bg-background text-right"
                    placeholder="SAR"
                  />
            ...
            <div className="mt-3 pt-3 border-t border-border">
              <FieldRow label="Overhead %">
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    value={active.overheadPercent}
                    onChange={e =>
                      updateField("overheadPercent", Number(e.target.value))
                    }
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {active.isApproved
                ? "Selected Working Scenario"
                : "Mark as Working Scenario"}
            </Button>
          ...
          {gpPct < 10 && gpPct > 0 && (
            <SignalCard
              type="critical"
              message={`GP% at ${gpPct.toFixed(1)}% — critically low`}
              recommendation="Review pricing assumptions before quote"
            />
          )}
          {gpPct >= 10 && gpPct < 22 && (
            <SignalCard
              type="warning"
              message={`GP% at ${gpPct.toFixed(1)}% — below 22% target`}
              recommendation="Consider margin improvement opportunities"
            />
          )}
          ...
          {versions.length >= 2 && (
            <Section
              ...
              icon={<Scale className="w-4 h-4 text-[#0b73ff]" />}
              badge={
                  ...
                  <thead>
                    <tr className="border-b border-border">
                      ...
                      {versions.map(v => (
                        <th
                          key={v.id}
                          className={`text-right py-2 px-2 font-semibold ${v.id === activeId ? "text-[#075eea]" : "text-muted-foreground"}`}
                        >
                          {v.name} {v.isApproved && "✓"}
                        </th>
                      ))}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {unverified > 0 && (
            <Badge
              variant="outline"
              className="text-[9px] border-amber-200 text-amber-600"
            >
      ...
      {data.length === 0 ? (
        <SignalCard
          type="info"
          message="No cost inputs captured"
          recommendation="Add external quotes, vendor rates, and finance assumptions"
        />
      ) : (
        <div className="space-y-2">
          {data.map((c, i) => (
            <div
              key={i}
              className={`p-3 rounded-lg border ${c.verified ? "border-emerald-200 bg-emerald-50/20" : "border-border"}`}
            >
              <div className="grid grid-cols-[1fr_1fr_100px_auto] gap-2 items-center">
                ...
                <input
                  ...
                  placeholder="SAR"
                  className="px-2 py-1.5 text-sm rounded border border-border bg-background text-right"
                />
              ...
              <input
                ...
                placeholder="Source: e.g. vendor quote, finance team..."
                className="w-full mt-1.5 px-2 py-1 text-xs rounded border border-border bg-background"
              />
            </div>
          ))}
        </div>
...
export function PricingLinesTab({
  ...
  const add = () =>
    onChange([
      ...data,
      { service: "", unit: "", rate: 0, quantity: 0, frequency: "", total: 0 },
    ]);
  const remove = (i: number) => onChange(data.filter((_, j) => j !== i));
  const update = (i: number, field: string, val: any) => {
  ...
  return (
    <div className="space-y-3">
      ...
      {data.length === 0 ? (
        <SignalCard
          type="info"
          message="No pricing lines defined"
          recommendation="Define storage, handling, transport, and VAS pricing"
        />
      ) : (
        <div className="overflow-x-auto">
                ...
                <tr key={i} className="border-b border-border/50">
                  <td className="py-1.5">
                    <input
                      ...
                      placeholder="Service"
                      className="w-full px-1 py-1 text-xs rounded border border-border bg-background"
                    />
                  </td>
                  <td className="py-1.5">
                    <input
                      ...
                      placeholder="Unit"
                      className="w-20 px-1 py-1 text-xs rounded border border-border bg-background"
                    />
                  </td>
                  <td className="py-1.5">
                    <input
                      ...
                      className="w-20 px-1 py-1 text-xs rounded border border-border bg-background text-right"
                    />
                  </td>
                  <td className="py-1.5">
                    <input
                      ...
                      className="w-16 px-1 py-1 text-xs rounded border border-border bg-background text-right"
                    />
                  </td>
                  <td className="py-1.5">
                    <input
...
export function MarginScenariosTab({
  ...
  const update = (i: number, field: string, val: any) => {
    ...
    if (field === "revenue" || field === "cost") {
      s.gp = s.revenue - s.cost;
      s.gpPercent = s.revenue > 0 ? (s.gp / s.revenue) * 100 : 0;
    }
        ...
        {data.map((s, i) => (
          <div key={i} className="p-3 rounded-lg border border-border">
            <div className="flex items-center justify-between mb-2">
              ...
              <Badge
                variant="outline"
                className={`text-[9px] ${s.gpPercent >= 22 ? "text-emerald-600 border-emerald-200" : s.gpPercent >= 10 ? "text-amber-600 border-amber-200" : s.gpPercent > 0 ? "text-red-600 border-red-200" : ""}`}
              >
            ...
            <div className="grid grid-cols-3 gap-2">
              <div>
                ...
                <input
                  ...
                  className="w-full px-2 py-1.5 text-sm rounded border border-border bg-background text-right"
                />
              </div>
              <div>
                <label className="text-[10px] text-muted-foreground">
              ...
              <div>
                ...
                <div
                  className={`px-2 py-1.5 text-sm rounded border border-border bg-muted/20 text-right font-medium ${s.gp >= 0 ? "text-emerald-700" : "text-red-700"}`}
                >
                  {formatSAR(s.gp)}
                </div>
              </div>
            ...
            <input
              ...
              className="w-full mt-2 px-2 py-1 text-xs rounded border border-border bg-background"
            />
          </div>
        ))}
      </div>
