/**
 * SC-01 W02-T03 clean-owned replacement — Pricing Lines Table + Review Modal.
 * Renders pricing lines supplied by the workspace bundle (props only; no own
 * transport; bundle source: commercial_tickets.type_details.pricing).
 * Empty and populated states are mutually exclusive: zero lines renders an
 * explicit empty notice and no totals.
 */
...
import {
  Eye, X, CheckCircle2, AlertTriangle, BarChart3, TrendingUp, Info,
} from "lucide-react";
...
type ReviewStatus = "Draft Mock" | "Needs Ops Input" | "Needs Finance Input" | "Reviewed Mock" | "Risk Flagged";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <Card className="w-full max-w-lg border shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        <CardContent className="pt-3 space-y-3 max-h-[70vh] overflow-y-auto">
          ...
          {line.description && <div className="text-xs"><span className="text-muted-foreground font-medium">Description:</span> <span>{line.description}</span></div>}
          {line.assumption && <div className="text-xs"><span className="text-muted-foreground font-medium">Assumption:</span> <span>{line.assumption}</span></div>}
          ...
          {(line.riskLevel === "Critical" || line.riskLevel === "High") && (
            <div className="p-2 rounded border border-orange-200 bg-orange-50 text-xs text-orange-800 flex items-center gap-2">
              ...
              Advisory flag: this line is recommended for human review. Nothing is blocked.
            </div>
          )}
          ...
          <div className="flex items-center gap-2 pt-2 border-t border-dashed">
            <Button variant="outline" size="sm" className="text-xs h-8 gap-1.5" onClick={async () => {
              if (!workspaceId) { toast.error("Line review: no workspace context — nothing was recorded."); onClose(); return; }
              const result = await markPricingLineReviewedMock(line.id, workspaceId, line.serviceName, actor);
              if (result.success) { toast.success(`"${line.serviceName}" review recorded.`); onActionComplete?.(); }
...
export default function CommercialPricingLinesTable({ lines, scenarioName, workspaceId, onActionComplete }: Props) {
  ...
  // Empty state is exclusive: no totals, no table, no counts alongside it.
  if (lines.length === 0) {
    return (
      <Card className="border shadow-none">
        <CardContent className="p-4">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-sm font-semibold">No pricing lines recorded</p>
              <p className="text-xs text-muted-foreground mt-0.5">
                No pricing lines exist for {scenarioName || "this scenario"} in the commercial ticket source.
              </p>
            </div>
          </div>
        </CardContent>
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="text-[10px] text-muted-foreground flex items-center gap-1">
          <Info className="w-3 h-3" /> Cost base: Ops/Finance · Selling: Commercial · Advisory display
        </div>
      </div>
    ...
    return (
      <Card className="border shadow-none">
        <CardContent className="p-4 text-sm text-muted-foreground">
          No scenarios recorded to compare.
        </CardContent>
      </Card>
    );
  }
