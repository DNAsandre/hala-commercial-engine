/**
 * SC-01 W02-T03 clean-owned replacement — Pricing Posture Panel.
 * Renders posture data supplied by the workspace bundle (props only; no own
    ...
    <Card className="border shadow-none">
      ...
      <CardContent className="pt-3 space-y-3">
        ...
        <div className={`rounded-lg p-2 ${pc.bg} ${pc.border} border`}>
          ...
          <p className={`text-[10px] mt-0.5 ${pc.text}`}>{p.rationale || "No rationale recorded."}</p>
        </div>
        ...
        {signalGroups.length > 0 ? (
          ...
          <p className="text-[10px] text-muted-foreground">No posture signals recorded.</p>
        )}
        ...
        <div>
          ...
          {p.recommendedActions.length > 0 ? (
            ...
            <p className="text-[10px] text-muted-foreground">No recommended actions recorded.</p>
          )}
        </div>
        ...
        {isRed && p.mockEscalationCreated && (
          <div className="p-2 rounded border border-red-200 bg-red-50 text-xs text-red-800 flex items-center gap-2">
            ...
            {p.severity === "Critical" ? "Critical Pricing Posture" : "High Pricing Risk"} — advisory escalation flagged. Nothing is blocked.
          </div>
        )}
        {!isRed && p.severity === "Medium" && (
          <div className="p-2 rounded border border-amber-200 bg-amber-50 text-xs text-amber-800 flex items-center gap-2">
            ...
            Amber Review — protect price and confirm Ops capacity. Advisory only.
          </div>
        )}

        {/* Advisory note */}
        <div className="text-[10px] text-muted-foreground bg-muted/20 rounded-lg p-2">
          <span className="font-medium">Advisory signal:</span> pricing posture guides quote review. It does not enforce decisions or sync with CRM/Finance.
        </div>

        {/* Actions */}
        <div className="flex items-center gap-2 pt-2 border-t border-dashed">
          <Button variant="outline" size="sm" className="text-xs h-7 gap-1" onClick={() => mkAction("pricing_posture_reviewed_mock", "Pricing posture reviewed", `Pricing posture (${p.posture}) reviewed. No enforcement.`, "PRICING_POSTURE_REVIEWED_MOCK", "Reviewed")}>
            <Eye className="w-3 h-3" /> Review Posture
          </Button>
          <Button variant="outline" size="sm" className="text-xs h-7 gap-1 text-blue-700 border-blue-200" onClick={() => mkAction("pricing_posture_bypass_mock", "Pricing posture advisory acknowledged", "Continue — pricing posture signal acknowledged; no enforcement exists.", "PRICING_POSTURE_BYPASS_MOCK", "Acknowledged")}>
