/**
 * SC-01 W02-T03 clean-owned replacement — SLA Control Tab.
 * Reads verified SLA drafts from commercial_tickets.type_details via
 * useCommercialWorkspaceData. Advisory display: no enforcement of any kind.
 * Empty and populated states are mutually exclusive.
 */
import { useState } from "react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { FileText, AlertTriangle, Eye, Play, ShieldAlert, ChevronDown, ChevronRight, Info, ClipboardCheck, BarChart3, Scale, Loader2, Database, AlertCircle } from "lucide-react";
...
const getRiskBg = (sla: CommercialSlaDraft) => sla.riskLevel === "Critical" ? "border-l-red-500" : sla.riskLevel === "High" ? "border-l-red-400" : sla.riskLevel === "Medium" ? "border-l-amber-400" : "border-l-emerald-400";

function SlaCard({ s, selected, onSelect }: { s: CommercialSlaDraft; selected: boolean; onSelect: () => void }) {
  return (
    <Card className={`border shadow-none transition-all cursor-pointer border-l-4 ${getRiskBg(s)} ${selected ? "ring-2 ring-[var(--color-hala-navy)] bg-muted/20" : "hover:bg-muted/10"}`} onClick={onSelect}>
      <CardContent className="p-3">
        ...
        {s.pricingLockStatus === "Not Locked" || s.pricingLockStatus === "Future Lock Required" ? (
          <div className="mt-2 p-2 rounded-md border border-red-200 bg-red-50 flex items-center gap-2"><ShieldAlert className="w-3.5 h-3.5 text-red-600 shrink-0" /><p className="text-[10px] text-red-700">Advisory: SLA should reflect locked pricing and agreed commercial terms. Warning only — nothing is blocked.</p></div>
        ) : null}
      </CardContent>
    </Card>
  );
...
function SlaDetailPanel({ s, slaBundle, workspaceId, onActionComplete }: { s: CommercialSlaDraft; slaBundle?: SlaBundleForDraft; workspaceId?: string; onActionComplete?: () => void }) {
  ...
  const runAction = async (label: string, fn: (id: string, ws: string, name: string, a: ActionActor) => Promise<{ success: boolean; error?: string }>, successMsg: string) => {
    if (!workspaceId) { toast.error(`${label}: no workspace context — nothing was recorded.`); return; }
    const result = await fn(s.id, workspaceId, s.slaName, actor);
    if (result.success) { toast.success(successMsg); onActionComplete?.(); }
    else { toast.error(`Advisory action could not be recorded: ${result.error}`); }
  };
      ...
      <Card className="border shadow-none">
        ...
        <CardContent className="pt-3 space-y-4">
          ...
          <div className="flex items-center gap-2 pt-2 border-t border-dashed flex-wrap">
            <Button variant="outline" size="sm" className="text-xs h-8 gap-1.5" onClick={() => runAction("SLA review", markSlaReviewedMock, "SLA review recorded.")}><Eye className="w-3.5 h-3.5" /> Review SLA</Button>
            <Button variant="outline" size="sm" className="text-xs h-8 gap-1.5" onClick={() => runAction("Ops review request", requestSlaOpsReviewMock, "Ops review request recorded.")}><ClipboardCheck className="w-3.5 h-3.5" /> Request Ops Review</Button>
            <Button variant="outline" size="sm" className="text-xs h-8 gap-1.5" onClick={() => runAction("Legal review request", requestSlaLegalReviewMock, "Legal review request recorded.")}><Scale className="w-3.5 h-3.5" /> Request Legal Review</Button>
            <Button variant="outline" size="sm" className="text-xs h-8 gap-1.5" onClick={() => mkAction("sla_preview_requested", "SLA Preview Requested", `SLA preview requested for "${s.version}". No document was generated.`, "SLA_PREVIEW_REQUESTED", "Preview Requested")}><FileText className="w-3.5 h-3.5" /> Request SLA Preview</Button>
            <Button variant="outline" size="sm" className="text-xs h-8 gap-1.5 text-blue-700 border-blue-200" onClick={() => mkAction("sla_advisory_acknowledged", "SLA Advisory Acknowledged", "Continue — SLA signals acknowledged; no enforcement exists.", "SLA_ADVISORY_ACKNOWLEDGED", "Acknowledged")}><Play className="w-3.5 h-3.5" /> Continue</Button>
          </div>
        </CardContent>
      </Card>
      ...
      {gaps.length > 0 && (
        <Card className="border shadow-none border-l-4 border-l-amber-400">
          ...
          <CardContent className="pt-3 space-y-2">
            <div className="text-[10px] text-muted-foreground bg-muted/20 rounded-lg p-2">Promise gaps show where SLA commitments may differ from pricing, capacity, or operational reality. Advisory display only.</div>
            {gaps.map(g => (
...
export default function CommercialSlaControlTab({ workspaceId, customerName }: Props) {
  ...
  const isSupabaseBacked = Boolean(bundle?.supabaseBacked) && drafts.length > 0;

  const [selectedId, setSelectedId] = useState<string>("");
  const [detailExpanded, setDetailExpanded] = useState(true);
  const selected = drafts.find(d => d.id === selectedId) || drafts[0];
  const selectedSlaBundle = selected ? bundle?.slaBundles.find(sb => sb.draft.id === selected.id) : undefined;

  if (status === "loading") {
    return (
      <div className="flex items-center justify-center py-20 gap-2">
        <Loader2 className="w-5 h-5 animate-spin text-[var(--color-hala-navy)]" />
        <span className="text-sm text-muted-foreground">Loading SLA data…</span>
      </div>
    );
  }
      ...
      <Card className="border-2 border-red-200 shadow-none bg-red-50/50">
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-sm font-semibold text-red-800 mb-0.5">Could not load SLA data</div>
              <p className="text-xs text-red-700">{errorMessage || "Unknown error"}</p>
              <Button variant="outline" size="sm" className="mt-2 text-xs" onClick={reload}>Retry</Button>
            </div>
          </div>
        </CardContent>
      </Card>
  ...
  // Empty state — exclusive: no counts, no cards, no comparison.
  if (drafts.length === 0) {
    return (
      <div className="space-y-4">
        {header}
        <Card className="border shadow-none bg-muted/20">
          <CardContent className="p-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-sm font-semibold mb-0.5">No verified SLA drafts for this workspace</div>
                <p className="text-xs text-muted-foreground leading-relaxed">No SLA drafts exist in the commercial ticket source for this workspace. This is an empty source — not a failed load.</p>
              </div>
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        {drafts.map(d => <SlaCard key={d.id} s={d} selected={selected?.id === d.id} onSelect={() => setSelectedId(d.id)} />)}
      </div>
      {selected && (
        <div>
      ...
      <div className="p-3 rounded-lg bg-blue-50 border border-blue-200 flex items-start gap-2"><Info className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" /><p className="text-xs text-blue-800">SLA records come from verified commercial ticket lineage. Advisory display only — nothing here blocks progression.</p></div>
    </div>
  );
}
