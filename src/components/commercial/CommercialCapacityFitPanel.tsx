/**
 * SC-01 W02-T03 clean-owned replacement — Capacity Fit + Warehouse Constraint Panel.
 * Renders capacity data supplied by the workspace bundle (props only; no own transport).
      ...
      <CardContent className="pt-3 space-y-3">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
          {[
            ...
            { label: "Ops Owner", value: cap.opsOwner ? cap.opsOwner.split(" · ")[0] : "Not assigned", color: "" },
          ].map(m => (
        ...
        <div>
          ...
          {cap.constraints.length > 0 ? (
            ...
            <p className="text-[10px] text-muted-foreground">No warehouse constraints recorded.</p>
          )}
        </div>
        ...
        {cap.promiseGaps.length > 0 && (
          <div className="rounded-lg border border-amber-200 bg-amber-50/50 p-2 space-y-1">
            ...
            <p className="text-[9px] text-amber-600 mb-1">Promise gaps show where quoted assumptions may differ from operational reality. Advisory display only.</p>
        ...
        {isRed && cap.mockEscalationCreated && (
          <div className="p-2 rounded border border-red-200 bg-red-50 text-xs text-red-800 flex items-center gap-2">
            ...
            Red Capacity Signal — advisory escalation flagged for Operations / Commercial Director review. Nothing is blocked.
          </div>
        )}
        {!isRed && cap.riskLevel === "Medium" && (
          <div className="p-2 rounded border border-amber-200 bg-amber-50 text-xs text-amber-800 flex items-center gap-2">
            ...
            Amber Capacity Review — acceptable with planning confirmation. Advisory only.
          </div>
        )}

        {/* Advisory note */}
        <div className="text-[10px] text-muted-foreground bg-muted/20 rounded-lg p-2">
          <span className="font-medium">Advisory signal:</span> capacity fit informs quote review. It does not block any action or sync with warehouse systems.
        </div>

        {/* Actions */}
        <div className="flex items-center gap-2 pt-2 border-t border-dashed flex-wrap">
          <Button variant="outline" size="sm" className="text-xs h-7 gap-1" onClick={() => mkAction("capacity_reviewed_mock", "Capacity reviewed", `Capacity for ${cap.customerName} reviewed. Utilization ${cap.utilizationAfter}%.`, "CAPACITY_REVIEWED_MOCK", "Reviewed")}>
            <Eye className="w-3 h-3" /> Review Capacity
          </Button>
          <Button variant="outline" size="sm" className="text-xs h-7 gap-1" onClick={() => mkAction("ops_input_requested_mock", "Ops input requested", `Ops input requested for ${cap.customerName}. Advisory log entry only.`, "OPS_INPUT_REQUESTED_MOCK", "Ops Input Requested")}>
            <UsersIcon className="w-3 h-3" /> Request Ops Input
          </Button>
          <Button variant="outline" size="sm" className="text-xs h-7 gap-1" onClick={() => mkAction("capacity_marked_reviewed_mock", "Capacity marked reviewed", `Capacity marked reviewed for ${cap.customerName}.`, "CAPACITY_MARKED_REVIEWED_MOCK", "Reviewed")}>
            <Eye className="w-3 h-3" /> Mark Reviewed
          </Button>
          <Button variant="outline" size="sm" className="text-xs h-7 gap-1 text-blue-700 border-blue-200" onClick={() => mkAction("capacity_bypass_mock", "Capacity advisory acknowledged", `Continue — capacity signal acknowledged for ${cap.customerName}; no enforcement exists.`, "CAPACITY_BYPASS_MOCK", "Acknowledged")}>
