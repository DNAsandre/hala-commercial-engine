/**
 * SC-01 W02-T03 clean-owned replacement — Commercial Audit Trail Tab.
 * Verified governance evidence log. Source: commercial_ticket_audit via the
 * workspace bundle (useCommercialWorkspaceData).
 * Empty and populated states are mutually exclusive: the "no events" notice is
 * rendered ONLY when zero events exist, never beside live counts.
 */
...
import { ShieldCheck, Info, Eye, Clock, User, Hash, Loader2, Database, AlertCircle } from "lucide-react";
import { type CommercialAuditEvent } from "@/lib/commercial-workspace-data";
...
const catBadge = (c: string) => {
  ...
  if (["PROPOSAL", "NEGOTIATION", "SLA"].includes(c)) return "text-[#075eea] bg-[#075eea]/10 border-[#075eea]/20";
...
export default function CommercialAuditTrailTab({ workspaceId }: Props) {
  ...
  const allEvents = bundle?.auditEvents ?? [];
  const isSupabaseBacked = Boolean(bundle?.supabaseBacked) && allEvents.length > 0;
      ...
      <Card className="border-2 border-red-200 shadow-none bg-red-50/50">
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-sm font-semibold text-red-800 mb-0.5">Could not load audit data</div>
  ...
  const header = (
    <div className="flex items-center justify-between">
      <div><h3 className="text-base font-serif font-bold flex items-center gap-2"><ShieldCheck className="w-5 h-5 text-[var(--color-hala-navy)]" /> Audit Trail</h3><p className="text-xs text-muted-foreground">Verified governance evidence log</p></div>
  ...
  // Empty state — exclusive: no counts, no filters, no table.
        ...
        <Card className="border shadow-none bg-muted/20">
          <CardContent className="p-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-sm font-semibold mb-0.5">No verified audit events found</div>
                <p className="text-xs text-muted-foreground">This tab stays empty until backend-backed audit events exist for this workspace. An empty list here means no events were captured — not a failed load.</p>
              </div>
            </div>
          </CardContent>
        </Card>
    ...
    <div className="space-y-4">
      ...
      <Card className="border shadow-none">
        <CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full text-xs">
          <thead><tr className="border-b text-muted-foreground bg-muted/30">{["Event Code", "Event Name", "Category", "Entity", "Actor", "Before", "After", "Severity", "Source", "Timestamp", "Trace", "Action"].map(h => <th key={h} className="text-left py-2 px-2 font-semibold whitespace-nowrap">{h}</th>)}</tr></thead>
          <tbody>{filtered.length > 0 ? filtered.map(e => (
            <tr key={e.id} className="border-b last:border-0 hover:bg-muted/10 transition-colors">
              ...
              <td className="py-1.5 px-2">{e.mock ? <Badge variant="outline" className="text-[8px] bg-slate-50 border-slate-200">Advisory</Badge> : "Verified"}</td>
            ...
            <tr><td colSpan={12} className="py-3 px-2 text-muted-foreground">No events match the current filters.</td></tr>
          )}</tbody>
        </table></div></CardContent>
      </Card>

      <div className="p-3 rounded-lg bg-blue-50 border border-blue-200 flex items-start gap-2"><Info className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" /><p className="text-xs text-blue-800">Only backend-backed audit events are shown here.</p></div>

      {/* Detail modal */}
      <Dialog open={!!selected} onOpenChange={() => setSelected(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle className="text-sm font-serif flex items-center gap-2"><ShieldCheck className="w-4 h-4 text-[var(--color-hala-navy)]" /> Audit Detail</DialogTitle></DialogHeader>
          {selected && (
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                {selected.mock && <Badge variant="outline" className="text-[9px] bg-slate-50 border-slate-200">Advisory</Badge>}
