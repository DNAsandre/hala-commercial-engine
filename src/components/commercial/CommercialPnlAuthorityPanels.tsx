/**
 * SC-01 W02-T03 clean-owned replacement — P&L Snapshot + Margin Authority Panels.
 * Renders P&L data supplied by the workspace bundle (props only; no own transport;
 * bundle source: commercial_tickets.type_details.pricing).
...
import {
  BarChart3, ShieldAlert, AlertTriangle, CheckCircle2, Eye, Play,
} from "lucide-react";
...
const confColors: Record<PnlConfidence, string> = {
  ...
  "Needs Ops Input": "text-[#075eea] bg-[#075eea]/10 border-[#075eea]/20",
      ...
      <CardContent className="pt-3 space-y-3">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-dashed">
          ...
          <Button variant="outline" size="sm" className="text-xs h-7 gap-1" onClick={async () => {
            if (!workspaceId) { toast.error("P&L review: no workspace context — nothing was recorded."); return; }
            const result = await markPnlReviewedMock(scenarioId ?? "unknown", workspaceId, scenarioName ?? "Unknown Scenario", actor);
            if (result.success) { toast.success("P&L review recorded. No production approval triggered."); onActionComplete?.(); }
            else { toast.error(`Advisory action could not be recorded: ${result.error}`); }
          }}>
            <Eye className="w-3 h-3" /> Review P&L
          </Button>
        </div>
      </CardContent>
    </Card>
...
export function MarginAuthorityPanel({ signal, workspaceId, scenarioName, onActionComplete }: { signal: MarginAuthoritySignal; workspaceId?: string; scenarioName?: string; onActionComplete?: () => void }) {
  ...
  const mkAction = async (eventType: string, title: string, desc: string, code: string, after: string) => {
    ...
    const result = await logMockAction(
      { workspaceId, eventType, title, description: desc, category: "Margin", actor, severity: "Info", relatedArtifact: scenarioName ?? "Unknown Scenario", relatedModule: "Margin Authority" },
      { workspaceId, eventCode: code, eventName: title, description: desc, category: "MARGIN", actor, entityType: "Margin Authority", entityName: scenarioName ?? "Unknown Scenario", beforeState: `${signal.gpPercent.toFixed(1)}% GP`, afterState: after, severity: "Info" }
    );
    ...
    <Card className="border shadow-none">
      ...
      <CardContent className="pt-3 space-y-3">
          ...
          <div>
            ...
            {signal.mockEscalationCreated && (
              <p className={`text-[10px] mt-1 font-medium ${sev.text}`}>
                Advisory escalation flagged — nothing is blocked.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
          ...
          <div><span className="text-muted-foreground">Advisory Override</span><p className="font-medium">{signal.allowTestBypass ? "Available" : "N/A"}</p></div>
        </div>
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-dashed">
          <Button variant="outline" size="sm" className="text-xs h-7 gap-1" onClick={() =>
            mkAction("margin_reviewed_mock", "Margin authority reviewed",
              `Margin authority reviewed for ${scenarioName ?? "Unknown Scenario"}. GP ${signal.gpPercent.toFixed(1)}%. No production approval.`,
              "MARGIN_REVIEWED_MOCK", "Reviewed")}>
            <Eye className="w-3 h-3" /> Mark Margin Reviewed
          </Button>
          <Button variant="outline" size="sm" className="text-xs h-7 gap-1 text-blue-700 border-blue-200" onClick={() =>
            mkAction("margin_bypass_mock", "Margin advisory acknowledged",
              `Continue — margin signal acknowledged; no enforcement exists. GP ${signal.gpPercent.toFixed(1)}%.`,
              "MARGIN_BYPASS_MOCK", "Acknowledged")}>
