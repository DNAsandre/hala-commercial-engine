/**
 * SC-01 W02-T03 clean-owned replacement — Commercial Activity Tab.
 * Verified operational timeline. Source: commercial_ticket_audit via the
...
import { Activity, Clock, Info, User, Loader2, Database, AlertCircle } from "lucide-react";
...
export default function CommercialActivityTab({ workspaceId }: Props) {
      ...
      <Card className="border-2 border-red-200 shadow-none bg-red-50/50">
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-sm font-semibold text-red-800 mb-0.5">Could not load activity data</div>
  ...
  // Empty state — exclusive: no counts, no filters, no timeline.
        ...
        <Card className="border shadow-none bg-muted/20">
          <CardContent className="p-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-sm font-semibold mb-0.5">No verified activity events found</div>
                <p className="text-xs text-muted-foreground">This tab stays empty until backend-backed workflow events exist for this workspace. An empty list here means no events were captured — not a failed load.</p>
              </div>
            </div>
          </CardContent>
        </Card>
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        {filtered.length > 0 ? (
          ...
          <p className="text-xs text-muted-foreground p-2">No events match the current filters.</p>
        )}
      </div>

      <div className="p-3 rounded-lg bg-blue-50 border border-blue-200 flex items-start gap-2"><Info className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" /><p className="text-xs text-blue-800">Only backend-backed events are shown here.</p></div>
    </div>
  );
}
