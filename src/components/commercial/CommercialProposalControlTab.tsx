/**
 * SC-01 W02-T03 clean-owned replacement — Proposal Control / Management Tab.
 * Reads verified proposal versions from commercial_tickets.type_details via
...
import {
  FileText, Eye, ArrowLeftRight, Loader2, Database, AlertCircle, Info,
} from "lucide-react";
...
const statusColor = (s: string) => {
  ...
  if (s.includes("Negotiation")) return "text-[#075eea] bg-[#075eea]/10 border-[#075eea]/20";
      ...
      <CardContent className="pt-3 space-y-3">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
          {[
            ...
            { label: "Margin Delta", value: p.marginDeltaFromQuote === 0 ? "No change" : `${p.marginDeltaFromQuote > 0 ? "+" : ""}${p.marginDeltaFromQuote}%`, color: p.marginDeltaFromQuote < 0 ? "text-red-600" : p.marginDeltaFromQuote > 0 ? "text-emerald-600" : "" },
            ...
            <div key={item.label} className="bg-muted/30 rounded-lg p-2">
              ...
              <p className={`text-xs font-bold mt-0.5 ${("color" in item && item.color) || ""}`}>{item.value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center gap-2 pt-1 border-t border-dashed">
          <Button variant="outline" size="sm" className="text-xs h-7 gap-1" onClick={async () => {
            if (!workspaceId) { toast.error("Proposal review: no workspace context — nothing was recorded."); return; }
            const result = await markProposalReviewedMock(p.id, workspaceId, p.proposalName, actor);
            if (result.success) { toast.success("Proposal review recorded."); onActionComplete?.(); }
            else { toast.error(`Advisory action could not be recorded: ${result.error}`); }
          }}>
            <Eye className="w-3 h-3" /> Review Proposal
          </Button>
          <Button variant="outline" size="sm" className="text-xs h-7 gap-1" onClick={() => mkAction("proposal_revision_requested", "Revision Requested", `Revision requested from "${p.version}". No document was created.`, "PROPOSAL_REVISION_REQUESTED", "Revision Requested")}>
            <ArrowLeftRight className="w-3 h-3" /> Request Revision
          </Button>
          <Button variant="outline" size="sm" className="text-xs h-7 gap-1" onClick={() => mkAction("proposal_pdf_requested", "PDF Requested", `PDF requested for "${p.version}". No document was generated.`, "PROPOSAL_PDF_REQUESTED", "PDF Requested")}>
            <FileText className="w-3 h-3" /> Request PDF
          </Button>
        </div>
      </CardContent>
    </Card>
        ...
        {rounds.map(r => (
          <div key={r.id} className="rounded-lg border p-2.5 space-y-1.5">
            ...
            <div className="grid grid-cols-4 gap-2 text-xs">
              <div><span className="text-muted-foreground">Pricing Change</span><p className="font-medium">{r.pricingChange}</p></div>
              <div><span className="text-muted-foreground">Margin Change</span><p className="font-medium">{r.marginChange}</p></div>
...
export default function CommercialProposalControlTab({ workspaceId, customerName }: Props) {
  ...
  const isSupabaseBacked = Boolean(bundle?.supabaseBacked) && proposals.length > 0;
      ...
      <Card className="border-2 border-red-200 shadow-none bg-red-50/50">
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-sm font-semibold text-red-800 mb-0.5">Could not load proposal data</div>
  ...
  const header = (
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        ...
        {proposals.length > 0 && <Badge variant="outline" className="text-[9px]">{proposals.length} version{proposals.length !== 1 ? "s" : ""}</Badge>}
      </div>
    </div>
  );

  // Empty state — exclusive: no version rows, no detail, no comparison.
        ...
        <Card className="border shadow-none bg-muted/20">
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-sm font-semibold mb-0.5">No verified proposal versions for this workspace</div>
                <p className="text-xs text-muted-foreground">No proposal versions exist in the commercial ticket source for this workspace. This is an empty source — not a failed load.</p>
              </div>
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {proposals.map(p => (
          <ProposalVersionRow key={p.id} p={p} selected={selected?.id === p.id} onSelect={() => setSelectedId(p.id)} />
        ))}
      </div>
