 ...
 *   - SLA and renewal ticket intake are paused by process isolation
 *   - Source metadata is captured for traceability while the build flow stays open
 */
...
type IntakeTicketType = Exclude<UnifiedTicketType, "renewal" | "sla">;
...
const INTAKE_TICKET_TYPES = (Object.keys(TICKET_TYPE_CONFIG) as UnifiedTicketType[]).filter(
  (ticketType): ticketType is IntakeTicketType =>
    ticketType !== "renewal" && ticketType !== "sla"
);

function normalizeDefaultTicketType(ticketType?: UnifiedTicketType): IntakeTicketType {
  return ticketType && INTAKE_TICKET_TYPES.includes(ticketType as IntakeTicketType)
    ? (ticketType as IntakeTicketType)
    : "proposal";
}
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => set("ticket_type", v as IntakeTicketType)}
              >
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {INTAKE_TICKET_TYPES.map((t) => (
          ...
          <div className="space-y-4">
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
              <p className="text-xs text-amber-800">
                <strong>Source truth matters.</strong> Capture where the proposal
                came from so the workspace can stay traceable while the flow remains open.
              </p>
            </div>
