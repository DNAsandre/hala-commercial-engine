...
describe("createQuote", () => {
  it("sends the full widened column set, including both sides of every mirror pair", async () => {
    ...
    // `updated_at` / `updated_by` are NOT stamped by this module and must stay
    // that way until a live probe confirms them: naming a column that does not
    // exist makes PostgREST reject the entire statement, so adding them
    // speculatively would break every quote write rather than just one field.
...
describe("updateQuote", () => {
  it("never stamps updated_at / updated_by on an edit either", async () => {
    queue({ data: [QUOTE_ROW], error: null });
