...
import { connectedSourceIdentity, type ConnectedRecordKind, type DocumentSource } from "../document-source";
...
export async function loadCommercialTicketPack(
  ...
  const recordKind: ConnectedRecordKind = source.kind === "proposal_engine" ? "proposal" : "tender";
  const identity = connectedSourceIdentity(recordKind);

  if (!tenderId) {
    // Safe error snapshot via the loader's own error path.
    return loadTenderPack("", source.packType as PackType, source.scenarioId, recordKind);
  }
  ...
  return {
    ...
    linked_entity_type: identity.linked_entity_type,
