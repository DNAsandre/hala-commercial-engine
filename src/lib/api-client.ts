...
function disabledSlaMutation(): Promise<ApiResponse<any>> {
  return Promise.reject(new Error(getSlaProcessIsolationReason()));
}
...
export const api = {
  ...
  slas: {
    listAll: () => Promise.resolve({ data: getEmptySlaRecords<any>(), count: 0 }),
    listByWorkspace: (_workspaceId: string) =>
      Promise.resolve({ data: getEmptySlaRecords<any>(), count: 0 }),
    get: (_id: string) => disabledSlaMutation(),
    create: (_workspaceId: string, _data: Record<string, any>) => disabledSlaMutation(),
    update: (_id: string, _data: Record<string, any>) => disabledSlaMutation(),
    submit: (_id: string) => disabledSlaMutation(),
    markOperationalReview: (_id: string) => disabledSlaMutation(),
    approve: (_id: string) => disabledSlaMutation(),
    reject: (_id: string, _reason: string) => disabledSlaMutation(),
    createVersion: (_id: string, _changeReason: string) => disabledSlaMutation(),
  },

  contractStatus: {
    get: (_workspaceId: string) =>
      Promise.resolve({ data: getEmptyContractStatuses<any>()[0] ?? null, count: 0 }),
    update: (_workspaceId: string, _data: Record<string, any>) => disabledSlaMutation(),
  },
  ...
  documents: {
    ...
    generatePdf: (data: { workspace_id: string; document_type: "quote" | "proposal"; source_id: string; source_version?: number; language?: string; notes?: string }) =>
