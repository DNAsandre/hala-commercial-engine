-- DRY RUN DELETE PASS 3 - V2 POST-DELETE CHECKS
-- Uses ONLY rows already archived in legacy_data_quarantine batch:
--   legacy_proposal_escalation_slop_20260521
--
-- This deletes inside a transaction, stores counts in a temp table,
-- checks live remaining rows AFTER the delete statements, then ROLLS BACK.
--
-- Run with RLS unchanged.

BEGIN;

CREATE TEMP TABLE delete_pass_3_dry_run_counts (
...
INSERT INTO delete_pass_3_dry_run_counts(check_name, rows)
SELECT 'deleted_tasks_dry_run', count(*) FROM deleted_tasks;
...
FROM delete_pass_3_dry_run_counts
ORDER BY check_name;
