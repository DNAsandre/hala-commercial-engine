 ...
 * No mock data. No hardcoding. No proposal heuristics.
 */

import { useState, useMemo, useEffect } from "react";
import {
  Search, X, Gavel, ChevronRight, ExternalLink,
  MapPin, User, RefreshCw, Loader2, Calendar,
} from "lucide-react";
...
function formatSar(n: number | null): string {
  if (n == null) return "[placeholder data for value]";
...
function formatStage(s: string | null): string {
  if (!s) return "[placeholder data for stage]";
...
const SUBMITTED_PHASES = new Set(["submitted", "clarification", "technical_review", "commercial_review"]);
  ...
  return (
    <div onClick={onClick} className="group rounded-lg border bg-card hover:shadow-md cursor-pointer transition-all duration-150 p-4">
      ...
      <div className="flex items-center justify-between gap-2 mb-1">
        <span className="text-sm font-semibold truncate flex-1">{t.customer_name || "[placeholder data for customer]"}</span>
        <Badge variant="outline" className="text-[9px] border-red-300 text-red-700 bg-red-50 shrink-0">Tender</Badge>
      </div>

      {/* Row 2: Title */}
      <p className="text-xs text-muted-foreground truncate mb-2">{t.title || "[placeholder data for title]"}</p>
      ...
      <div className="flex items-center gap-1.5 flex-wrap mb-2">
        {t.crm_pipeline_stage ? (
          ...
          <Badge variant="outline" className="text-[9px] border-dashed border-slate-300 text-muted-foreground/50">[placeholder data for CRM stage]</Badge>
        )}
        {t.phase ? (
          <Badge variant="outline" className="text-[9px] border-indigo-300 text-indigo-700 bg-indigo-50">{formatStage(t.phase)}</Badge>
        ) : (
          <Badge variant="outline" className="text-[9px] border-dashed border-slate-300 text-muted-foreground/50">[placeholder data for phase]</Badge>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-[10px] mb-2">
        ...
        <div>
          ...
          <span className={`font-mono font-semibold text-xs ${gpColor}`}>{gpPct != null ? `${gpPct}%` : "—"}</span>
        </div>
        <div>
      ...
      <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
        <div className="flex items-center gap-1">
          ...
          <span className="truncate max-w-[90px]">{t.assigned_owner || "[placeholder data for owner]"}</span>
        </div>
        <div className="flex items-center gap-1">
          <MapPin className="w-3 h-3" />
          <span>{t.region || "[placeholder data for region]"}</span>
        </div>
...
function TenderPreviewPopup({ tender: t, open, onClose }: { tender: TenderRow | null; open: boolean; onClose: () => void }) {
  ...
  const facts: { label: string; value: string }[] = [
    ...
    { label: "GP%", value: t.target_gp_percent != null ? `${t.target_gp_percent}%` : "[placeholder data for GP%]" },
    { label: "Probability", value: t.probability_percent != null ? `${t.probability_percent}%` : "[placeholder data for probability]" },
    { label: "Owner", value: t.assigned_owner || "[placeholder data for owner]" },
    { label: "Region", value: t.region || "[placeholder data for region]" },
    { label: "Source", value: t.source || "[placeholder data for source]" },
    { label: "Submission Deadline", value: t.submission_deadline || "[placeholder data for deadline]" },
    { label: "Days to Deadline", value: days != null ? (days > 0 ? `${days} days` : days === 0 ? "Today" : `${Math.abs(days)} days overdue`) : "[placeholder data for days to deadline]" },
    { label: "Days in Status", value: t.days_in_status != null ? `${t.days_in_status}d` : "[placeholder data for days in status]" },
  ];
      ...
      <DialogContent className="max-w-lg p-0 gap-0">
        ...
        <div className="p-5 border-b">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-base font-semibold">{t.customer_name || "[placeholder data for customer]"}</h3>
            <Badge variant="outline" className="text-[9px] border-red-300 text-red-700 bg-red-50">Tender</Badge>
          </div>
          <p className="text-xs text-muted-foreground mt-0.5">{t.title || "[placeholder data for title]"}</p>
          ...
          <div className="flex items-center gap-1.5 mt-2 flex-wrap">
            {t.crm_pipeline_stage ? (
              ...
              <Badge variant="outline" className="text-[9px] border-dashed text-muted-foreground/50">[placeholder data for CRM stage]</Badge>
            )}
            {t.phase ? (
              <Badge variant="outline" className="text-[9px] border-indigo-300 text-indigo-700 bg-indigo-50">Phase: {formatStage(t.phase)}</Badge>
            ) : (
              <Badge variant="outline" className="text-[9px] border-dashed text-muted-foreground/50">[placeholder data for phase]</Badge>
            )}
          </div>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          {/* Placeholder section for fields that don't exist yet */}
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">Tender Documents</p>
            <div className="flex items-center gap-2 flex-wrap">
              {["Technical Proposal", "Commercial Proposal", "Compliance Pack", "Submission Pack"].map(d => (
                <Badge key={d} variant="outline" className="text-[9px] border-dashed text-muted-foreground/50">
                  {d}: [placeholder data for {d.toLowerCase()}]
                </Badge>
              ))}
            </div>
          </div>
        ...
        <div className="p-4 border-t flex items-center justify-between">
          ...
          <Button size="sm" className="text-xs" onClick={() => { onClose(); navigate(`/tenders/${t.id}`); }}>
