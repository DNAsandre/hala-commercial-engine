...
function MilestoneStrip({
  ...
  const isTerminal = TENDER_TERMINAL.includes(current as TenderMilestone);
  const currentIdx = STRIP_MILESTONES.indexOf(current as any);
  const suggested = getSuggestedNextMilestones(current as TenderMilestone);
    ...
    <div>
      ...
      <div className="flex items-center gap-0 overflow-x-auto pb-1 scrollbar-thin">
        {STRIP_MILESTONES.map((m, i) => {
          ...
          const isSuggested = suggested.includes(m as TenderMilestone);

          return (
            <div key={m} className="flex items-center shrink-0">
              {/* Step */}
              <button
                onClick={() => !isCurrent && onMove(m as TenderMilestone)}
                disabled={isCurrent}
                title={isSuggested ? `Suggested next: ${getTenderStatusDisplayName(m as TenderMilestone)}` : getTenderStatusDisplayName(m as TenderMilestone)}
                className={`
              ...
              >
                ...
                <span className={`text-[10px] font-medium whitespace-nowrap leading-none ${
                  ...
                  {getTenderMilestoneShortLabel(m as TenderMilestone)}
                </span>
        ...
        <div className="ml-3 flex items-center gap-1">
          ...
          {(["lost_withdrawn"] as string[]).map(m => (
            <button
              key={m}
              onClick={() => current !== m && onMove(m as TenderMilestone)}
              disabled={current === m}
      ...
      <p className="text-[10px] text-muted-foreground/60 mt-1.5 italic">
        Tender process tracker — separate from internal working board.
            ...
            >
              <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                TENDER_TERMINAL.includes(m)
                  ? m === "closed_won" ? "bg-emerald-400" : m === "closed_lost" ? "bg-red-400" : "bg-gray-300"
                  : "bg-primary/40"
              }`} />
...
function TenderRiskChips({ tenderId, packCount }: { tenderId: string; packCount: number }) {
  ...
  return (
    <>
      ...
      {signal.gatesWouldBlockCount > 0 && <Badge variant="outline" className="text-[8px] h-4 px-1 border-amber-300 text-amber-700 bg-amber-50" title="Readiness signals flagged for review — advisory only">Signals {signal.gatesWouldBlockCount}</Badge>}
...
function CreateTenderForm({
  ...
  function handleSubmit() {
    ...
    const tender = createTender({
      ...
      source,
      region,
      executionRegions: [], targetSites: [], executionType: '', geographicComplexity: '', siteCount: 0, executionNotes: '',
      notes,
      crmPipelineStage: "prospecting" as any,
    });
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-serif font-bold">Tender Workspace</h1>
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => navigate("/crm_pipeline")}>
                ...
                <TabsContent value="overview" className="mt-0">
                  ...
                  <div className="px-4 pt-4 pb-3 border-b border-border">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <Button variant="outline" size="sm" className="text-xs h-7" onClick={() => navigate(`/crm_pipeline`)}>
