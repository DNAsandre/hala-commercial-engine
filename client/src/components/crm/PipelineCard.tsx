      ...
      >
        <div className="p-2.5 space-y-1.5">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Badge variant="outline" className={`text-[8px] h-4 px-1 shrink-0 ${t.ticketType === "tender" ? "border-red-300 text-red-700 bg-red-50" : "border-indigo-300 text-indigo-700 bg-indigo-50"}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            {t.ecrScore >= 0 && (
              <div className="flex items-center gap-0.5 ml-auto">
                <div className={`w-1.5 h-1.5 rounded-full ${t.ecrScore >= 70 ? "bg-emerald-500" : t.ecrScore >= 40 ? "bg-amber-500" : "bg-red-400"}`} />
                <span className="text-[8px] text-muted-foreground">ECR {t.ecrScore}</span>
              </div>
            )}
          </div>
          ...
          {/* Row 5: Next action */}
          {t.nextAction && <p className="text-[8px] text-indigo-600 font-medium truncate">→ {t.nextAction}</p>}
        ...
        <div className="px-8 py-5 border-b border-border bg-muted/20 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-2 mb-3">
            {t.ticketType === "tender" ? (
              ...
              <span className="text-xs font-bold uppercase tracking-widest text-indigo-600">Proposal Opportunity</span>
            )}
          </div>
          <div className="flex items-center gap-3 mb-2">
            <span className="text-xl font-semibold">{t.customerName}</span>
            <Badge variant="outline" className={`text-xs px-2 py-0.5 ${t.ticketType === "tender" ? "border-red-300 text-red-700 bg-red-50" : "border-indigo-300 text-indigo-700 bg-indigo-50"}`}>
              {t.ticketType === "tender" ? "Tender" : "Proposal"}
            </Badge>
        ...
        {t.ticketType === "proposal" ? (
          <div className="px-8 py-5 border-b border-border">
            <div className="grid grid-cols-4 gap-4">
              <GaugeMini label="ECR Score" value={t.ecrScore} />
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (t.ticketType === "tender") {
                ...
                navigate(`/workspaces/${t.workspaceId || t.id}`);
              }
            }}
          >
            <ExternalLink className="w-4 h-4" />
            {t.ticketType === "tender" ? "Open Tender Workspace" : "Open Workspace"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
