...
export default function TenderDocumentsLibrary({ ws, tenderId, reload, initialCategory = "all", initialStage = "all" }: TenderDocumentsLibraryProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return docs.filter(doc => {
      ...
      if (stage !== "all" && !doc.stage_relevance.includes(stage)) return false;
      ...
      <div className="grid gap-2 md:grid-cols-5">
        ...
        <Select value={stage} onValueChange={value => setStage(value as TenderStageRelevance | "all")}>
          ...
          <SelectContent>
            ...
            {TENDER_STAGE_RELEVANCE.map(item => <SelectItem key={item} value={item}>{item}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={doc.id} className="border-t hover:bg-muted/30">
                  ...
                  <td className="px-3 py-2">
                    <div className="flex max-w-[260px] flex-wrap gap-1">{doc.stage_relevance.length ? doc.stage_relevance.map(item => <Badge key={item} variant="outline" className="text-[9px]">{item}</Badge>) : <span className="text-muted-foreground">-</span>}</div>
                  </td>
