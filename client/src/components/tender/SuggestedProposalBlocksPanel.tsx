/**
 * SuggestedProposalBlocksPanel — Foundation Preview Component
 *
 * Shows what proposal blocks a given stage can feed in the future.
 *
 ...
 *   - Target PDF Studio block types (informational only)
 *   - Future output use
 */
...
import {
  ...
  AlertCircle, ArrowRight, Lock, FileText, Info,
} from "lucide-react";
import {
  type StageKey,
      ...
      <CardHeader className="pb-2 border-b border-border bg-muted/20">
        ...
        {!open && (
          <p className="text-[10px] text-muted-foreground mt-1 pl-[30px]">
            Future proposal sections this stage can feed once required data is captured.
          </p>
        )}
      </CardHeader>
      ...
      {open && (
        <CardContent className="p-4 space-y-3">
          ...
          <div className="flex items-start gap-2 p-2.5 rounded-lg border border-blue-200/60 bg-blue-50/40">
            ...
            <div>
              <p className="text-[10px] text-blue-700 font-medium">Foundation Preview</p>
              <p className="text-[9px] text-blue-600/70 mt-0.5">
                Shows which proposal blocks this stage can feed. Drafting is disabled until the Proposal Block Workbench sprint.
              </p>
            </div>
          </div>
              ...
              <div className="flex flex-wrap gap-1">
                {mapping.futureUse.map(fu => (
                  <Badge
                    key={fu}
                    variant="outline"
                    className="text-[8px] border-indigo-200 bg-indigo-50 text-indigo-600"
                  >
