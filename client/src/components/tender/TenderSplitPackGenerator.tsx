...
export default function TenderSplitPackGenerator({ ws, onClose }: { ws: TenderWorkspace; onClose: () => void }) {
  ...
  function handleGenerateOutput() {
    toast.warning("Document output generation is disabled here.", {
      description: "Use PDF Studio only after real document generation is wired to verified records.",
    });
  }
          ...
          {checksRun && (
            <div>
              ...
              <Button variant="outline" size="sm" className="text-xs h-8 gap-1.5" onClick={handleGenerateOutput}>
                <FileOutput className="w-3.5 h-3.5" /> Open PDF Studio
              </Button>
              <p className="text-[10px] text-muted-foreground mt-1">Document creation is blocked here until PDF Studio is wired to verified source records.</p>
            </div>
          )}
