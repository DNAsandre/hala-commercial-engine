...
export default function InternalReviewStage({ ws, activeTab, reload }: StageProps) {
  if (activeTab === "review_dashboard")
    return <InternalReviewDashboardTab ws={ws} reload={reload} />;
  if (activeTab === "operations_review")
    return <DepartmentalReviewTab ws={ws} department="ops" requiredVolumes={["Technical", "Shared"]} reload={reload} />;
  if (activeTab === "finance_review")
    return <DepartmentalReviewTab ws={ws} department="finance" requiredVolumes={["Commercial", "Shared"]} reload={reload} />;
  if (activeTab === "legal_review")
    return <DepartmentalReviewTab ws={ws} department="legal" requiredVolumes={["Technical", "Commercial", "Shared", "Appendix"]} reload={reload} />;
  if (activeTab === "exceptions")
    return <InternalReviewExceptionsTab ws={ws} reload={reload} />;
