 ...
 *
 * Replaces old "Tender Execution Scope" with a structured capture form.
 * Feeds: PDF Studio, pricing, compliance, proposal generation.
 *
 * 10 Subsections:
 *   1. Scope Summary
 ...
 *  10. Internal Notes
 *  + PDF Studio Usage Map (read-only)
 *
...
import {
  ...
  emptySowData,
  PDF_STUDIO_WIRING_MAP,
} from "@/lib/sow-pdf-studio-wiring";
...
function ChipMultiSelect({
  options, selected, onToggle, accentColor = "indigo",
}: {
        ...
        return (
          <label key={opt}
            className={`flex items-center gap-2 px-2.5 py-1.5 rounded-md border text-xs cursor-pointer transition-all ${
              sel ? "bg-indigo-50 border-indigo-200 text-indigo-800" : "bg-white border-slate-200 text-slate-600 hover:bg-slate-50"
            }`}>
            <input type="checkbox" checked={sel} onChange={() => onToggle(opt)} className="accent-indigo-600 w-3.5 h-3.5" />
            {opt}
          </label>
        );
      })}
...
function SectionHeader({
  ...
  return (
    <button type="button" onClick={onToggle}
      ...
      {collapsed ? <ChevronRight className="w-4 h-4 text-slate-400 group-hover:text-indigo-500 transition-colors" /> : <ChevronDown className="w-4 h-4 text-indigo-500" />}
...
export default function ScopeOfWorkCapture({ ws, reload }: Props) {
  ...
  return (
    <Card className="border-border shadow-none border-l-4 border-l-indigo-500">
      <CardHeader className="pb-2 border-b border-border bg-indigo-50/30">
        <CardTitle className="text-sm font-semibold flex items-center gap-2">
          ...
          <span className="text-[9px] text-muted-foreground/60 ml-auto font-normal">
            Structured operational scope used by PDF Studio, pricing, compliance, and proposal generation.
          </span>
        </CardTitle>
      </CardHeader>
      <CardContent className="p-4 space-y-6">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full min-h-[80px] px-3 py-2 rounded-md border border-slate-200 text-xs resize-y bg-white focus:outline-none focus:ring-1 focus:ring-indigo-400"
          />
        </div>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Value"
                    className="w-full h-8 px-2 rounded-md border border-slate-200 text-xs bg-white focus:outline-none focus:ring-1 focus:ring-indigo-400"
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full min-h-[50px] px-3 py-2 rounded-md border border-slate-200 text-xs resize-y bg-white focus:outline-none focus:ring-1 focus:ring-indigo-400"
                />
              </div>
            </div>
          )}
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full min-h-[70px] px-3 py-2 rounded-md border border-slate-200 text-xs resize-y bg-white focus:outline-none focus:ring-1 focus:ring-indigo-400"
                  />
                </div>
                <div>
        ...
        <div>
          ...
          <p className="text-[9px] text-muted-foreground/60 mb-1.5">
            Internal-only notes. Not used directly in PDF Studio unless selected.
          </p>
          <textarea
        ...
        {/* ── PDF Studio Usage Map ──────────────────────────── */}
        <div className="border border-slate-200 rounded-lg overflow-hidden">
          <div className="px-4 py-2.5 bg-blue-50/30 border-b border-slate-200 flex items-center gap-2">
            <Info className="w-3.5 h-3.5 text-blue-500" />
            <span className="text-xs font-semibold text-blue-700">PDF Studio Usage Map</span>
            <Badge variant="outline" className="text-[9px] border-blue-200 bg-blue-50 text-blue-600 ml-auto">Read-only</Badge>
          </div>
          <div className="p-0">
            <table className="w-full text-[10px]">
              <thead>
                <tr className="border-b border-slate-200 bg-muted/10">
                  <th className="text-left p-2 font-semibold text-muted-foreground">SOW Field</th>
                  <th className="text-left p-2 font-semibold text-muted-foreground">Block / Token</th>
                  <th className="text-left p-2 font-semibold text-muted-foreground">Proposal Section</th>
                </tr>
              </thead>
              <tbody>
                {PDF_STUDIO_WIRING_MAP.map((entry, idx) => (
                  <tr key={idx} className="border-b border-slate-100">
                    <td className="p-2 font-medium">{entry.sowField}</td>
                    <td className="p-2 text-muted-foreground font-mono">
                      {entry.blockKey || entry.semanticToken || "—"}
                    </td>
                    <td className="p-2 text-muted-foreground">{entry.proposalSection}</td>
                  </tr>
                ))}
              </tbody>
            </table>
