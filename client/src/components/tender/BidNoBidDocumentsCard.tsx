/**
 * BidNoBidDocumentsCard — Compact Stage Document Card
 *
 * Reads from the global Tender Document Library only.
 * Filters by stage_relevance containing "bid_no_bid" or "Bid / No-Bid".
...
export default function BidNoBidDocumentsCard({ ws, onOpenDocuments }: Props) {
  const docs = ws.documents || [];

  // Filter documents relevant to Bid / No-Bid stage
  const relevantDocs = docs.filter(d => {
    ...
    if (typeof relevance === "string") {
      ...
      return lower.includes("bid_no_bid") || lower.includes("bid / no-bid") || lower.includes("bid/no-bid");
    }
  ...
  const sourceCount = relevantDocs.filter(d => d.document_category === "Source").length;
  const supportingCount = relevantDocs.filter(d => d.document_category === "Supporting").length;
    ...
    <Card className="border-border shadow-none">
      <CardHeader className="pb-2 border-b border-border bg-muted/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold">Bid / No-Bid Documents</span>
          <Badge variant="outline" className="text-[8px] ml-auto">
            {relevantDocs.length} documents
          </Badge>
        </div>
      </CardHeader>
