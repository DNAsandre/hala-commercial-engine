/**
 * SolutionDesignDocumentsCard — Compact Stage Document Card
 *
 * Reads from the global Tender Document Library only.
 * Filters by stage_relevance containing "solution_design" or "Solution Design".
...
export default function SolutionDesignDocumentsCard({ ws, onOpenDocuments }: Props) {
  ...
  const relevantDocs = docs.filter(d => {
    ...
    if (typeof relevance === "string") {
      ...
      return lower.includes("solution_design") || lower.includes("solution design") || lower.includes("solutioning");
    }
    ...
    <Card className="border-border shadow-none">
      <CardHeader className="pb-2 border-b border-border bg-muted/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold">Solution Design Documents</span>
