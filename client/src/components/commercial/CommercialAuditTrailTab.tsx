 ...
 * Verified governance evidence log shell.
 * SUPA-003B compatibility remains, but no local audit fallback is used.
 */
...
import { ShieldCheck, Info, Eye, Clock, User, Hash, Loader2, Database } from "lucide-react";
...
export default function CommercialAuditTrailTab({ workspaceId }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="border shadow-none bg-muted/20"><CardContent className="p-3"><div className="flex items-start gap-3"><Info className="w-5 h-5 text-blue-600 shrink-0 mt-0.5" /><div><div className="text-sm font-semibold mb-0.5">No verified audit events found</div><p className="text-xs text-muted-foreground">This tab will stay empty until backend-backed audit events exist for this workspace.</p></div></div></CardContent></Card>
      ...
      <div className="p-3 rounded-lg bg-blue-50 border border-blue-200 flex items-start gap-2"><Info className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" /><p className="text-xs text-blue-800">Only backend-backed audit events are shown here. Empty means not captured yet.</p></div>
        ...
        <DialogContent className="max-w-lg">
          ...
          {selected && (
            <div className="space-y-3">
              <div className="p-2 rounded-lg border border-blue-200 bg-blue-50 text-xs text-blue-800 flex items-center gap-2"><Info className="w-3.5 h-3.5" /> This audit record is shown only when it exists in the backend source.</div>
