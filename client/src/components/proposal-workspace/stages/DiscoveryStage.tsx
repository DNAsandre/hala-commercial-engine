      ...
      {data.map(note => (
        <div key={note.id} className="border border-border rounded-lg bg-background">
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-muted/30 transition-colors rounded-t-lg"
          >
            <div className="flex items-center gap-2">
              <Calendar className="w-3.5 h-3.5 text-muted-foreground" />
              <span className="text-sm font-medium">{note.date || "No date"}</span>
            ...
            <button onClick={e => { e.stopPropagation(); removeNote(note.id); }} className="text-muted-foreground/40 hover:text-red-500 transition-colors">
