 ...
 *   - ECR → displayed only when present on canonical tickets
 *   - Health Score → composite of real DSO + real payment + ECR + risks
 */
...
function computeHealthScore(row: {
  ...
  // ECR contribution (0-15)
  if (row.avgEcr >= 70) score += 15;
  else if (row.avgEcr >= 40) score += 7;
...
export function deriveCustomerRowsFromTickets(
  ...
  for (const [name, custTickets] of groups) {
    ...
    const ecrScores = custTickets.map(t => t.ecrScore).filter(v => v >= 0);
    const avgEcr = ecrScores.length > 0 ? Math.round(ecrScores.reduce((s, v) => s + v, 0) / ecrScores.length) : 0;
