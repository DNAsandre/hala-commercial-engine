/**
 * Tender Workspace Data — Type definitions + UI helpers
 *
 * SUPA-008: Types, labels, and UI helpers in this file are actively used.
 * No hardcoded tender workspace records live here.
 * Reads should go through supabase-tender-data.ts and writes through
...
export interface TenderMockGate {
  ...
  isMock: boolean;
  wouldBlock: boolean;
  /** Advisory alias for wouldBlock — used in signal-doctrine UI */
  needsReview?: boolean;
...
export function getGateEnforcementLabel(e: GateEnforcement): string {
  return ({ mock_only: "Advisory Only", warn_future: "Advisory Warning", would_enforce_production: "Would Enforce in Production" })[e];
}
...
export const TENDER_STAGE_RELEVANCE: TenderStageRelevance[] = ["Identified", "Qualification", "Bid / No-Bid", "Pricing", "Solutioning", "Proposal Drafting", "Review", "Approval", "Submission", "Award", "Post Submission"];
...
export function getTenderDocumentStatusColor(s: TenderDocumentStatus): string {
  return ({
    ...
    Submitted: "text-indigo-700 bg-indigo-50 border-indigo-200",
  })[s];
}
...
export function stageLabelFromInternalStage(stage: string): TenderStageRelevance {
  return ({
    ...
    pnl_pricing: "Pricing",
    tender_drafting: "Proposal Drafting",
  ...
  } as Record<string, TenderStageRelevance>)[stage] ?? "Identified";
}
...
export function documentsForTenderStage(docs: TenderDocument[], stage: string): TenderDocument[] {
  ...
  return docs.filter(doc => doc.stage_relevance.includes(relevance));
}
...
const tenderWorkspaces: TenderWorkspace[] = [];
...
export function getTenderExecutionSignals(tenderId: string): TenderExecutionSignal[] {
  ...
  if (summary.gatesWouldBlock > 0) signals.push({ ...base, riskColor: summary.gatesWouldBlock > 5 ? "red" : "amber", riskReason: `${summary.gatesWouldBlock} readiness signals flagged for review`, nextAction: "Review Readiness Signals", signalType: "readiness" });
...
/**
 * Legacy compatibility stub. No default tender route is provided.
 */
export function getLegacyTenderId(): string {
  return "";
}
...
export function getGateStatusLabel(status: MockGateStatus): string {
  const labels: Record<MockGateStatus, string> = {
    ...
    warning: "Warning",
    fail: "Fail",
    would_block: "Review Recommended",
